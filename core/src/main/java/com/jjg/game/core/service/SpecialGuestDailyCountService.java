 ...
 * 广告次数按玩家当天全局累计，付费次数按卡池类型和商品配置累计；过期时间保留两天，既覆盖跨天结算窗口，
 * 也不会长期占用存储。</p>
 */
@Service
public class SpecialGuestDailyCountService {
    private static final String AD_COUNT = "simSpecialGuestAd:%d";
    private static final String PAID_COUNT = "simSpecialGuestPaid:%d:%d:%d";
    ...
    public int getPaidCount(long playerId, int poolType, int cfgId) {
        return countDao.getCount(PAID_COUNT.formatted(TimeHelper.getDayNumerical(), poolType, cfgId),
                String.valueOf(playerId)).intValue();
    }

    /** 增加一次购买次数并返回增加后的当天次数。 */
    public int addPaidCount(long playerId, int poolType, int cfgId) {
        return countDao.incrementWithoutExpireRefresh(
                PAID_COUNT.formatted(TimeHelper.getDayNumerical(), poolType, cfgId), String.valueOf(playerId),
                BigDecimal.ONE, EXPIRE_SECONDS).intValue();
    }
}
