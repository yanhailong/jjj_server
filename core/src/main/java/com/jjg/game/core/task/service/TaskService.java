...
public class TaskService implements IRedDotService, IPlayerLoginSuccess, GameEventListener {
    ...
    private CommonResult<TaskData> addNewTasks(long playerId, TaskData tmpData) {
        CommonResult<TaskData> result = new CommonResult<>(Code.FAIL);
...
//                playerTasks.fastPut(taskData.getConfigId(), taskData);
    ...
    public List<Task> getPlayerTaskList(long playerId, int type) {
        ...
        Map<Integer, TaskSortParam> map = new HashMap<>(taskData.getTaskDetails().size());
        //生成数据
        for (TaskDetail taskDetail : taskData.getTaskDetails().values()) {
            ...
            map.put(taskDetail.getConfigId(), new TaskSortParam(task, taskDetail, minProgress));
        }
        if (map.isEmpty()) {
            return taskList;
        }
