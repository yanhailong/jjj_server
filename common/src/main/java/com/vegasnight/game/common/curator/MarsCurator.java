...
import com.vegasnight.game.common.curator.MarsNodeListener.NodeChangeType;
...
public class MarsCurator implements TreeCacheListener {
    ...
    private void leaderLatch(){
        try{
            ...
            if(NodeType.ROOM.name().equals(nodeConfig.getType())){
                int[] gameTypes = nodeConfig.getGameTypes();
                if(gameTypes == null || gameTypes.length < 1){
                    log.warn("选举主节点失败,gameTypes错误 nodeName = {},nodeType = {},gameTypes={}",nodeConfig.getName(),nodeConfig.getType(),gameTypes == null ? 0 : gameTypes.length);
                    return;
                }

                for(int i=0;i<gameTypes.length;i++){
                    String path = mkPath("/" + nodeConfig.getParentPath()) + "/MASTER/" + nodeConfig.getType() + "/" + gameTypes[i];
            ...
            for(Map.Entry<Integer,String> en : pathMap.entrySet()){
                ...
                LeaderLatch latch = new LeaderLatch(client, path,nodeConfig.getName(), LeaderLatch.CloseMode.NOTIFY_LEADER);
                latch.addListener(new LeaderLatchListener() {
                    ...
                    public void isLeader() {
                        ...
                        nodeManager.update();
                        log.info("该节点被选举为主节点 nodeName = {},nodeType = {},gameChildType={},wight={}",nodeConfig.getName(),nodeConfig.getType(),gameChildType,nodeConfig.weight);
                    }
                    ...
                    public void notLeader() {
                        ...
                        log.info("该节点离开，失去主节点身份 nodeName = {},gameChildType={},nodeType = {}",nodeConfig.getName(),gameChildType,nodeConfig.getType());
                    }
                });
                latch.start();
            }
    ...
    private void notifyMarsNodeListener(NodeChangeType nodeChangeType, MarsNode marsNode) {
        ...
        if ((nodePath == null || !nodePath.equals(path)) && !rootPath.equals(path) && marsNode.getNodeConfig() != null) {
            marsNodeListeners.forEach((k,v) -> {
                ...
                log.info("notify mars node listeners.path={},nodeChangeType={},listener={}", path, nodeChangeType, v.getClass().getName());
            });
        }
    }
    ...
    private boolean add(String data,String path){
        try{
            ...
            if("master".equalsIgnoreCase(nodeType)){
                ...
                if(!NodeType.ROOM.name().equals(masterType)){
                    return true;
                }
