 ...
 * and the Spliterator provided in method {@link #spliterator()} are <em>not</em> guaranteed to
 * traverse the elements of the TakeSortPriorityBlockingQueue in any particular order. If you need
 * ordered traversal, consider using {@code Arrays.sort(pq.toArray())}. Also, method {@code drainTo}
 * can be used to <em>remove</em> some or all elements in priority order and place them in another
 * collection.
 *
 ...
 * href="{@docRoot}/java.base/java/util/package-summary.html#CollectionsFramework">Java Collections
 * Framework</a>.
 *
...
public class TakeSortPriorityBlockingQueue<E> extends AbstractQueue<E>
    implements BlockingQueue<E>, java.io.Serializable {
    /**
     *
     */
    private static final long serialVersionUID = -2909426009255963291L;
     ...
     * descendant d of n, n <= d. The element with the lowest value is in queue[0], assuming the queue
     * is nonempty.
     */
    ...
    /**
     * A plain PriorityQueue used only for serialization, to maintain compatibility with previous
     * versions of this class. Non-null only during serialization/deserialization.
     */
    private PriorityQueue<E> q;

    /**
     * Creates a {@code TakeSortPriorityBlockingQueue} with the default initial capacity (11) that
     * orders its elements according to their {@linkplain Comparable natural ordering}.
     */
    public TakeSortPriorityBlockingQueue() {
        this(DEFAULT_INITIAL_CAPACITY, null);
    }

    /**
     * Creates a {@code TakeSortPriorityBlockingQueue} with the specified initial capacity that orders
     ...
     */
    public TakeSortPriorityBlockingQueue(int initialCapacity) {
     ...
     * @throws IllegalArgumentException if {@code initialCapacity} is less than 1
     */
    public TakeSortPriorityBlockingQueue(int initialCapacity, Comparator<? super E> comparator) {
    ...
    /**
     * Creates a {@code TakeSortPriorityBlockingQueue} containing the elements in the specified
     ...
     */
    public TakeSortPriorityBlockingQueue(Collection<? extends E> c) {
        ...
        if (c instanceof SortedSet<?>) {
            ...
            heapify = false;
        } else if (c instanceof TakeSortPriorityBlockingQueue<?>) {
            TakeSortPriorityBlockingQueue<? extends E> pq =
                (TakeSortPriorityBlockingQueue<? extends E>) c;
            this.comparator = (Comparator<? super E>) pq.comparator();
            screen = false;
            if (pq.getClass() == TakeSortPriorityBlockingQueue.class) // exact match
                heapify = false;
        }
        Object[] es = c.toArray();
        int n = es.length;
        // If c.toArray incorrectly doesn't return Object[], copy it.
        if (es.getClass() != Object[].class) es = Arrays.copyOf(es, n, Object[].class);
        if (screen && (n == 1 || this.comparator != null)) {
            for (Object e : es) if (e == null) throw new NullPointerException();
        }
    ...
    private E dequeue() {
        ...
        if ((result = (E) ((es = queue)[0])) != null) {
            PlayerPriority playerPriority = ((BasePriorityHandlerRunnable) result).getPlayerPriority();
            if (playerPriority != null && playerPriority.getPriority() > 0) {
                heapify();
                result = (E) (queue[0]);
            }
            final int n;
     ...
     *
     * <p>To simplify and speed up coercions and comparisons, the Comparable and Comparator versions
    ...
    private static <T> void siftDownUsingComparator(
        ...
        while (k < half) {
            ...
            Object c = es[child];
            int right = child + 1;
            if (right < n && cmp.compare((T) c, (T) es[right]) > 0) c = es[child = right];
    ...
    private void heapify() {
        ...
        int n = size, i = (n >>> 1) - 1;
        final Comparator<? super E> cmp;
        if ((cmp = comparator) == null) for (; i >= 0; i--) siftDownComparable(i, (E) es[i], es, n);
        else for (; i >= 0; i--) siftDownUsingComparator(i, (E) es[i], es, n, cmp);
    }
    ...
    public boolean offer(E e) {
        ...
        int n, cap;
        Object[] es;
        while ((n = size) >= (cap = (es = queue).length)) tryGrow(es, cap);
        try {
            final Comparator<? super E> cmp;
            if ((cmp = comparator) == null) siftUpComparable(n, e, es);
            else siftUpUsingComparator(n, e, es, cmp);
            size = n + 1;
            notEmpty.signal();
        } finally {
            lock.unlock();
    ...
    /**
     * Always returns {@code Integer.MAX_VALUE} because a {@code TakeSortPriorityBlockingQueue} is not
    ...
    private int indexOf(Object o) {
        if (o != null) {
            final Object[] es = queue;
            for (int i = 0, n = size; i < n; i++) if (o.equals(es[i])) return i;
        }
        return -1;
    }
    ...
    private void removeAt(int i) {
        ...
        if (n == i) // removed last element
            ...
            es[n] = null;
            final Comparator<? super E> cmp;
            if ((cmp = comparator) == null) siftDownComparable(i, moved, es, n);
            else siftDownUsingComparator(i, moved, es, n, cmp);
            if (es[i] == moved) {
                if (cmp == null) siftUpComparable(i, moved, es);
                else siftUpUsingComparator(i, moved, es, cmp);
            }
        }
        size = n;
    }
    ...
    public void clear() {
        ...
        try {
            final Object[] es = queue;
            for (int i = 0, n = size; i < n; i++) es[i] = null;
            size = 0;
        } finally {
            lock.unlock();
        }
    ...
    final class Itr implements Iterator<E> {
        ...
        public void forEachRemaining(Consumer<? super E> action) {
            ...
            if ((i = cursor) < es.length) {
                lastRet = -1;
                cursor = es.length;
                for (; i < es.length; i++) action.accept((E) es[i]);
                lastRet = es.length - 1;
            }
        }
    }
     ...
     *
     * <p>For compatibility with previous version of this class, elements are first copied to a
     * java.util.PriorityQueue, which is then serialized.
     *
     * @param s the stream
     ...
     * ClassNotFoundException { try { s.defaultReadObject(); int sz = q.size();
     * SharedSecrets.getJavaObjectInputStreamAccess().checkArray(s, Object[].class, sz); this.queue
     * = new Object[Math.max(1, sz)]; comparator = q.comparator(); addAll(q); } finally { q = null;
     * } }
     */
    ...
    final class PBQSpliterator implements Spliterator<E> {
        ...
        private int getFence() {
            if (array == null) fence = (array = toArray()).length;
            return fence;
        }
    ...
    /**
     * Returns a {@link Spliterator} over the elements in this queue. The spliterator does not
     * traverse elements in any particular order (the {@link Spliterator#ORDERED ORDERED}
     * characteristic is not reported).
     *
     * <p>The returned spliterator is <a href="package-summary.html#Weakly"><i>weakly
     * consistent</i></a>.
     *
     * <p>The {@code Spliterator} reports {@link Spliterator#SIZED} and {@link Spliterator#NONNULL}.
     *
    ...
    private boolean bulkRemove(Predicate<? super E> filter) {
        ...
        try {
            ...
            for (i = beg + 1; i < end; i++) if (filter.test((E) es[i])) setBit(deathRow, i - beg);
            int w = beg;
    ...
    public void forEach(Consumer<? super E> action) {
        ...
        try {
            final Object[] es = queue;
            for (int i = 0, n = size; i < n; i++) action.accept((E) es[i]);
        } finally {
            lock.unlock();
        }
    }
    ...
    static {
        try {
            ...
            ALLOCATIONSPINLOCK =
                l.findVarHandle(TakeSortPriorityBlockingQueue.class, "allocationSpinLock", int.class);
