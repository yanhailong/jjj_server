...
public class SizeDiceTreasureSettlementPhase extends BaseDiceSettlementPhase<SizeDiceTreasureGameDataVo> {
    ...
    public void phaseDoAction() {
        ...
        if (currentPool != null) {
            ...
            if (result == null) {
                log.error("大小骰宝回收触发 生成结果失败 当前池:{} 标准池:{}", currentPool.getFirst(), currentPool.getSecond());
            } else {
                randomNumDice = result;
                log.info("大小骰宝回收触发 生成结果成功 当前池:{} 标准池:{}", currentPool.getFirst(), currentPool.getSecond());
            }
        }
