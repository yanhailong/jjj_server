...
@ProtobufMessage(
    ...
    resp = true,
    cmd = RussianLetteMessageConstant.RespMsgBean.RESP_RUSSIAN_LETTE_SUMMARY_LIST
)
@ProtoDesc("返回房间摘要信息")
public class RespRussianLetteSummaryList extends AbstractResponse {

    @ProtoDesc("房间摘要列表")
    public List<RussianLetteSummary> tableSummaryList;

    public RespRussianLetteSummaryList(int code) {
        super(code);
    }
}
