...
public class LuxuryCarClubSettlementPhase extends BaseSettlementPhase<LuxuryCarClubGameDataVo> {
    ...
    public void phaseDoAction() {
        ...
        // 中奖区域ID
        WinPosWeightCfg winPosWeightCfgs = winPosWeightCfgMap.get(randomRewardPosId);
        int clientShowPosId = winPosWeightCfgs.getWinPosID();
            // 对应的下注区域ID
        Integer betAreaId = winPosWeightCfgs.getBetArea().get(0);
        // 添加中奖记录
        ...
        for (Map.Entry<Long, GamePlayer> entry : gameDataVo.getGamePlayerMap().entrySet()) {
            ...
            // 给玩家进行结算
            long playerWin = calcGold(winPosWeightCfgs, playerBetTotal);
