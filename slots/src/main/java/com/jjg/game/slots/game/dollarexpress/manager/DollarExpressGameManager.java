...
//        generateLib(100000);
    ...
    public DollarExpressGameRunInfo startGame(PlayerController playerController, long betValue) {
        ...
        try {
            ...
            if (playerGameData == null) {
                log.debug("获取玩家游戏数据失败，开始游戏失败 playerId = {},gameType = {},wareId = {}", playerController.playerId(), playerController.player.getGameType(), playerController.player.getWareId());
            ...
            if (status == DollarExpressConstant.Status.NORMAL) {  //正常
                ...
                log.debug("当前正处于二选一状态，禁止开始游戏操作 playerId = {},gameType = {},wareId = {}, status = {}", playerController.playerId(), playerController.player.getGameType(), playerController.player.getWareId(), status);
                return gameRunInfo;
                ...
                log.debug("开始游戏失败，检测到错误状态 playerId = {},gameType = {},wareId = {},status = {}", playerController.playerId(), playerController.player.getGameType(), playerController.player.getWareId(), status);
                return gameRunInfo;
            }
    ...
    public DollarExpressGameRunInfo chooseFreeGameType(PlayerController playerController, int chooseStatus) {
        ...
        try {
            ...
            DollarExpressPlayerGameData playerGameData = getPlayerGameData(playerController);
            if (playerGameData == null) {
                log.debug("获取玩家游戏数据失败，二选一失败 playerId = {},gameType = {},wareId = {}", playerController.playerId(), playerController.player.getGameType(), playerController.player.getWareId());
                gameRunInfo.setCode(Code.NOT_FOUND);
                return gameRunInfo;
            }
            ...
            if (playerGameData.getStatus() == DollarExpressConstant.Status.NOTMAL_ALL_BOARD) {  //普通二选一
                ...
                gameRunInfo.setCode(Code.FORBID);
                log.debug("当前不处于二选一状态，禁止二选一操作 playerId = {},gameType = {},wareId = {},status = {}", playerController.playerId(), playerController.player.getGameType(), playerController.player.getWareId(), playerGameData.getStatus());
                return gameRunInfo;
            }

            log.info("玩家进行二选一，playerId = {},gameType = {},wareId = {},chooseStatus = {}", playerController.playerId(), playerGameData.getGameType(), playerController.player.getWareId(), chooseStatus);
            return gameRunInfo;
        } catch (Exception e) {
            log.error("", e);
        }
    ...
    private DollarExpressGameRunInfo normal(PlayerController playerController, DollarExpressGameRunInfo gameRunInfo, DollarExpressPlayerGameData playerGameData, long betValue) {
        ...
        if (player.getGold() < betValue) {
            log.debug("玩家余额不足，无法快乐的玩游戏 playerId = {},gameType = {},wareId = {},betValue = {},currentGold = {}", playerController.playerId(), playerController.player.getGameType(), playerController.player.getWareId(), betValue, player.getGold());
            gameRunInfo.setCode(Code.NOT_ENOUGHT);
            return gameRunInfo;
        }
