...
import com.jjg.game.slots.game.tigerbringsriches.pb.res.ResTigerBringsRichesEnterGame;
import com.jjg.game.slots.game.tigerbringsriches.pb.res.ResTigerBringsRichesPoolValue;
import com.jjg.game.slots.game.tigerbringsriches.pb.res.ResTigerBringsRichesStartGame;
 ...
 */
@Component
public class TigerBringsRichesGameSendMessageManager extends BaseSendMessageManager {

    private final AbstractTigerBringsRichesGameManager gameManager;
    private final SlotsLogger slotsLogger;

    public TigerBringsRichesGameSendMessageManager(TigerBringsRichesGameManager gameManager, SlotsLogger slotsLogger) {
    ...
    public void reqTigerBringsRichesEnterGame(PlayerController playerController, TigerBringsRichesGameRunInfo gameRunInfo) {
        ...
        ResTigerBringsRichesEnterGame res = new ResTigerBringsRichesEnterGame(Code.SUCCESS);
        if (config != null) {
            ...
            if (playerGameData.getStatus() == TigerBringsRichesConstant.Status.REAL_TIGER_BRINGS_RICHES) {
                if (playerGameData.getSpecialLib() != null) {
                    List<TigerBringsRichesResultLib> randomResult = playerGameData.getSpecialLib().getSpecialResult();
            ...
            for (int poolId : prizePoolIdList) {
                ...
                TigerBringsRichesPoolInfo poolInfo = new TigerBringsRichesPoolInfo();
                poolInfo.id = poolId;
    ...
    public void sendPoolMessage(PlayerController playerController, TigerBringsRichesGameRunInfo gameRunInfo) {
        SendInfo sendInfo = new SendInfo();
        ResTigerBringsRichesPoolValue res = new ResTigerBringsRichesPoolValue(gameRunInfo.getCode());
        if (gameRunInfo.success()) {
