 ...
 * @date 2026/2/4
 */
@Document(collection = "MoneyRabbitPlayerGameDataRoomDTO")
public class MoneyRabbitPlayerGameDataRoomDTO extends SlotsPlayerGameDataRoomDTO {
    //缓存免费的结果库
    private MoneyRabbitResultLib freeLib;
    //剩余的免费次数
    private int remainFreeCount;
    //当前的免费游戏数组中的下标值
    private int freeIndex;

    public MoneyRabbitResultLib getFreeLib() {
        return freeLib;
    }

    public void setFreeLib(MoneyRabbitResultLib freeLib) {
    ...
    public <T extends SlotsPlayerGameData> T converToGameData(Class<T> cla) throws Exception {
        ...
        if (t instanceof MoneyRabbitPlayerGameData playerGameData) {
