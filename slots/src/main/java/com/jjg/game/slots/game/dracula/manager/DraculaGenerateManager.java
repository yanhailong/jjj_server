...
import com.jjg.game.slots.game.dracula.data.DraculaAddFreeInfo;
...
import com.jjg.game.slots.game.dracula.data.DraculaWildBlessingInfo;
 ...
 */
@Component
public class DraculaGenerateManager extends AbstractSlotsGenerateManager<DraculaAwardLineInfo, DraculaResultLib> {
    public DraculaGenerateManager() {
    ...
    //
    private DraculaAddFreeInfo DraculaAddFreeInfo;
    //百搭赐福配置（playType=3）modeId_pickCount
    ...
    public DraculaResultLib checkAward(int[] arr, DraculaResultLib lib, boolean freeModel) throws Exception {
        if(freeModel){
            ...
            //检查满线图案
            List<DraculaAwardLineInfo> fullLineInfoList = fullLine(lib);
            ...
            //存储消除后添加的图标
            List<DraculaAddIconInfo> addIconInfoList = new ArrayList<>();
    ...
    @Override
    protected DraculaAwardLineInfo addFullLineAwardInfo(Set<Integer> sameIconIndexSet, BaseElementRewardCfg cfg, int[] arr) {
        DraculaAwardLineInfo info = super.addFullLineAwardInfo(sameIconIndexSet, cfg, arr);
        info.setSameIcon(cfg.getElementId().getFirst() % 10);
        return info;
    }

    @Override
    protected DraculaAwardLineInfo getAwardLineInfo() {
        return new DraculaAwardLineInfo();
    }
     ...
     * @return
     */
    private int checkAddFreeCount(DraculaResultLib lib) {
        //SpecialPlay 表里没有配 TYPE_ADD_FREE_COUNT 行时 DraculaAddFreeInfo 不会被初始化，按"不增加"处理
        if (this.DraculaAddFreeInfo == null) {
            return 0;
        }
        if (this.DraculaAddFreeInfo.getLibType() != DraculaConstant.SpecialMode.FREE) {
            return 0;
        }
        ...
        //统计盘面上目标图标（scatter）的总个数，按个数查档：4个→12、5个→14、6个→16
        int scatterCount = 0;
        for (int i = 1; i < lib.getIconArr().length; i++) {
            if (lib.getIconArr()[i] == this.DraculaAddFreeInfo.getTargetIcon()) {
                scatterCount++;
            }
        }
        if (scatterCount <= 0) {
        ...
        int addFree = this.DraculaAddFreeInfo.getAddFreeCount(scatterCount);
        if (addFree <= 0) {
            return 0;
        }
        int prop = this.DraculaAddFreeInfo.getProp(scatterCount);
        //万分比为 0 时视为必中（兼容简化配置）
    ...
    public void repairIcons(int libType, int[] arr, List<DraculaAwardLineInfo> list, List<DraculaAddIconInfo> addIconInfoList, int winCount) {
        ...
        //检查中奖
        List<DraculaAwardLineInfo> newAwardInfoList = fullLine(arr);
...
//        printResult(arr);
        ...
        for (int i = beginIndex; i <= endIndex; i++) {
            ...
            if (expandedRemovedIndexes.contains(i)) {
                ...
                if (replaceIcon != null) {
                    //把替换符号的占位卡在原 anchor 自身 space 内，避免单格金框换成多格 wild 时整列溢出
                    BaseElementCfg originalCfg = baseElementCfgMap.get(icon);
                    int originalSpace = (originalCfg == null || originalCfg.getSpace() <= 0) ? 1 : originalCfg.getSpace();
                    int before = validIndexes.size();
                    addIconWithPlaceholders(validIndexes, replaceIcon, originalSpace);
        ...
        //最终防护：理论上前面 cap 已经够，但若样本表里出现意外情况，
        //截断到列高 rows，至少不再抛 ArrayIndexOutOfBoundsException
        int rowCnt = baseInitCfg.getRows();
        if (validIndexes.size() > rowCnt) {
            log.warn("processIcons 列内图标超出 rows，截断 colIndex={}, size={}, rows={}", colIndex, validIndexes.size(), rowCnt);
            validIndexes = new ArrayList<>(validIndexes.subList(0, rowCnt));
            postChangeFlags = new ArrayList<>(postChangeFlags.subList(0, rowCnt));
        }
...
//        System.out.println();
    ...
    private void addIconWithPlaceholders(List<Integer> validIndexes, int icon) {
        addIconWithPlaceholders(validIndexes, icon, Integer.MAX_VALUE);
    }

    /**
     * 把 icon 及其占位符塞入 validIndexes，space 上限被 maxSpace 卡住。
     * 用于 postChange 场景：原 anchor 只让出 maxSpace 个格子，替换符号最多只能占这么多格。
     */
    private void addIconWithPlaceholders(List<Integer> validIndexes, int icon, int maxSpace) {
        BaseElementCfg cfg = baseElementCfgMap.get(icon);
        int space = cfg == null ? 1 : cfg.getSpace();
        if (space < 1) space = 1;
        if (space > maxSpace) space = Math.max(1, maxSpace);
    ...
    @Override
    public void calTimes(DraculaResultLib lib) throws Exception {
        //先结算常规游戏奖励（连线 + 消除后新增图标），文档要求：触发免费时，先结算常规奖励再触发免费
        lib.addTimes(calLineTimes(lib.getAwardLineInfoList()));
        lib.addTimes(calAfterAddIcons(lib.getAddIconInfos()));

        if (triggerFreeLib(lib, DraculaConstant.SpecialMode.FREE)) {
            //再叠加免费游戏奖励
     ...
     * @return
     */
    public int calLineTimes(List<DraculaAwardLineInfo> list) {
        if (list == null || list.isEmpty()) {
            return 0;
        }

        int times = 0;
        for (DraculaAwardLineInfo awardLineInfo : list) {
     ...
     * @return
     */
    public long calAfterAddIcons(List<DraculaAddIconInfo> addIconInfos) {
        if (addIconInfos == null || addIconInfos.isEmpty()) {
            return 0;
        }

        long times = 0;
        for (DraculaAddIconInfo info : addIconInfos) {
            if (info.getAwardLineInfoList() == null || info.getAwardLineInfoList().isEmpty()) {
                continue;
            }

            for (DraculaAwardLineInfo awardLineInfo : info.getAwardLineInfoList()) {
                times += awardLineInfo.getBaseTimes();
            }
        }
        return times;
    ...
    protected void specialPlayConfig() {
        ...
        for (Map.Entry<Integer, SpecialPlayCfg> en : GameDataManager.getSpecialPlayCfgMap().entrySet()) {
            ...
            //连续中奖
            if (cfg.getPlayType() == DraculaConstant.SpecialPlay.TYPE_CONSECUTIVE_WINS) {
            ...
            } else if (cfg.getPlayType() == DraculaConstant.SpecialPlay.TYPE_ADD_FREE_COUNT) {  //增加免费次数
                //格式：libType,targetIcon,count_addFreeCount_prop|count_addFreeCount_prop|...
                //例如 2,13,4_12_10000|5_14_10000|6_16_10000 表示免费模式下，
                //  scatter(id=13) 满 4/5/6 个分别触发 12/14/16 次免费游戏（万分比 10000）。
                String[] arr = cfg.getValue().split(",");
                if (arr.length < 3) {
                    log.warn("TYPE_ADD_FREE_COUNT 格式异常（至少需 3 段 libType,icon,groups），跳过 value={}", cfg.getValue());
                    continue;
                }
                DraculaAddFreeInfo tmpDraculaAddFreeInfo = new DraculaAddFreeInfo();
                tmpDraculaAddFreeInfo.setLibType(Integer.parseInt(arr[0]));
                tmpDraculaAddFreeInfo.setTargetIcon(Integer.parseInt(arr[1]));
                for (String group : arr[2].split("\\|")) {
                    String[] parts = group.split("_");
                    if (parts.length < 3) {
                        log.warn("TYPE_ADD_FREE_COUNT 组格式异常，跳过 group={}", group);
                        continue;
                    }
                    int scatterCount = Integer.parseInt(parts[0]);
                    int addFreeCount = Integer.parseInt(parts[1]);
                    int prop = Integer.parseInt(parts[2]);
                    tmpDraculaAddFreeInfo.put(scatterCount, addFreeCount, prop);
                }
                this.DraculaAddFreeInfo = tmpDraculaAddFreeInfo;
                log.info("TYPE_ADD_FREE_COUNT 加载完成 libType={}, targetIcon={}, tiers={}",
                        tmpDraculaAddFreeInfo.getLibType(),
                        tmpDraculaAddFreeInfo.getTargetIcon(),
                        tmpDraculaAddFreeInfo.getCountToAddFree());
            } else if (cfg.getPlayType() == DraculaConstant.SpecialPlay.TYPE_WILD_BLESSING) {  //百搭赐福
                //格式：modeId_pickCount，例如 3_3 表示 modeId=3 模式下从所有银框元素里随机抽 3 个变 wild
                String[] arr = cfg.getValue().split("_");
                if (arr.length < 2) {
                    log.warn("百搭赐福：TYPE_WILD_BLESSING 格式异常，跳过 value={}", cfg.getValue());
                    continue;
                }
                DraculaWildBlessingInfo info = new DraculaWildBlessingInfo();
                info.setModeId(Integer.parseInt(arr[0]));
                info.setPickCount(Integer.parseInt(arr[1]));
                this.wildBlessingInfo = info;
                log.info("百搭赐福配置加载完成 modeId={}, pickCount={}", info.getModeId(), info.getPickCount());
            }
        }
    ...
    public SpecialGirdInfo gridUpdate(int cfgId, int[] arr) {
        ...
        for (int i = 0; i < maxForCount; i++) {
            ...
            if (cfgId == DraculaConstant.SpecialGird.GRID_TWO
                    || cfgId == DraculaConstant.SpecialGird.GRID_THERE
                    || cfgId == DraculaConstant.SpecialGird.GRID_FOUR) {
