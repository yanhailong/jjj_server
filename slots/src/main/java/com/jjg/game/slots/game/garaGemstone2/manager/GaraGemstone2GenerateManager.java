...
//        int[] extended = appendMultiplyAxisIcons(originalArr, lib);
    ...
    private int[] appendMultiplyAxisIcons(int[] originalArr, GaraGemstone2ResultLib lib) {
        ...
        int centerPos;
        boolean isSyntheticIcon = selectedInfo.getIconId() == GaraGemstone2Constant.BaseElement.ID_JACKPOOL
                || selectedInfo.getIconId() == GaraGemstone2Constant.BaseElement.ID_WHEEL;
        if (isSyntheticIcon) {
            //JACKPOOL/WHEEL 的 icon 是代码合成的"虚拟符号"，roller elements 里本来就不存在，
            //直接随机取一个中心位置（前/后两格仍从 roller 取真符号），最后由后面的代码强写中格为合成 icon
