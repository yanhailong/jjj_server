...
public class MoneyRabbitMessageHandler {
    ...
    public void reqPoolValue(PlayerController playerController, ReqMoneyRabbitPool req) {
        try {
            ...
            MoneyRabbitGameRunInfo gameRunInfo;
            if (playerController.getScene() == null) {
                gameRunInfo = gameManager.getPoolValue(MoneyRabbitGameRunInfo.class, playerController, req.stakeVlue);
            } else if (playerController.getScene() instanceof SlotsRoomController) {
                gameRunInfo = roomGameManager.getPoolValue(MoneyRabbitGameRunInfo.class, playerController, req.stakeVlue);
            } else {
                log.warn("playerController.getScene() is error, scene={}", playerController.getScene());
                return;
            }
