...
import com.jjg.game.core.concurrent.priority.BasePriorityHandler;
import com.jjg.game.core.concurrent.priority.BasePriorityHandlerRunnable;
import com.jjg.game.core.concurrent.priority.PlayerPriority;
...
public abstract class BaseProcessor implements IPrintTimeHandler {
  ...
  public static final String LOG_DO_OVER_TIME = "[doovertime]--->";
  public static final String LOG_WAIT_OVER_TIME = "[waitovertime]--->";
          ...
          new Callable<String>() {
            ...
            public String call() {
              try {
                ...
                LoggerUtils.LOGGER.error(exStr);
  ...
  /** 向该线程投递Handler(Runnable) */
  public void executeHandlerImmediately(BaseHandler handler) {
    try {
      if (executor.isShutdown()) {
        LoggerUtils.LOGGER.error(
            "线程" + this.name + " 已经停止, Handler被丢弃: " + handler.getClass().getName());
      ...
      handler.setCreateAloneNum(createAloneNum.incrementAndGet());
      handler.setAloneNum(execAloneNum.get());
      execHandler(handler, "executeHandlerImmediately way");
      TOTAL_EXEC_NUM.incrementAndGet();
      execAloneNum.incrementAndGet();
    } catch (Exception e) {
      LoggerUtils.LOGGER.error(ExceptionEx.e2s(e));
  ...
  protected void execHandler(BaseHandler handler, String callWay) {
    try {
      ...
      try {
        ...
        LoggerUtils.LOGGER.error("处理handler后置逻辑时异常\n" + ExceptionEx.e2s(e));
      }
  ...
  public void printQueueSize(BaseHandler handler, int size) {
    ...
    if (timeErrorPrint) {
      LoggerUtils.LOGGER.error("线程队列长度超过警戒值:" + size + " " + this.name);
    } else {
      LoggerUtils.LOGGER.warn("线程队列长度超过警戒值:" + size + " " + this.name);
    }
  }
  ...
  public void printWaitTime(BaseHandler handler, long logDoTime, long waitTime, String callWay) {
    ...
    if (this.timeErrorPrint) {
      LoggerUtils.LOGGER.error(printContent);
    } else {
      LoggerUtils.LOGGER.warn(printContent);
    }
  }
  ...
  public void printDoTime(BaseHandler handler, long doTime, String info) {
    ...
    String printContent =
        LOG_DO_OVER_TIME
            ...
            + ",info:"
            + info;
    if (this.timeErrorPrint || (doTime > 2 * logDoTime)) {
      LoggerUtils.LOGGER.error(printContent);
    } else {
      LoggerUtils.LOGGER.warn(printContent);
    }
