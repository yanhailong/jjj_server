...
public class ToSouthFreeHandUtils {
    ...
    private static final Logger log = LoggerFactory.getLogger(ToSouthFreeHandUtils.class);
     ...
     * 判断牌型
     */
    public static ToSouthFreeCardType getCardType(List<Card> cards) {
        if (cards == null || cards.isEmpty()) return ToSouthFreeCardType.NONE;
        int size = cards.size();
        cards.sort(CARD_COMPARATOR);

        if (size == 1) return ToSouthFreeCardType.SINGLE;
        if (size == 2 && isPair(cards)) return ToSouthFreeCardType.PAIR;
        if (size == 3 && isTriple(cards)) return ToSouthFreeCardType.TRIPLE;
        if (size == 4 && isBombQuad(cards)) return ToSouthFreeCardType.BOMB_QUAD; // 四张是炸弹
        if (size >= 3 && isStraight(cards)) return ToSouthFreeCardType.STRAIGHT;
        if (size >= 6 && size % 2 == 0 && isConsecutivePairs(cards)) return ToSouthFreeCardType.CONSECUTIVE_PAIRS;

        return ToSouthFreeCardType.NONE;
    }
    ...
    public static boolean compare(List<Card> prev, List<Card> current) {
        ToSouthFreeCardType type1 = getCardType(prev);
        ToSouthFreeCardType type2 = getCardType(current);
        if (type2 == ToSouthFreeCardType.NONE) return false;
        // 炸弹牌型，只能炸2的牌型和三连对，不能炸顺子、单张对子3-A等普通牌型
        if (type2 == ToSouthFreeCardType.BOMB_QUAD) {
            if (type1 == ToSouthFreeCardType.BOMB_QUAD) {
                // 都是四张，比大小
                return compareMaxCard(prev, current);
            }
            // 规则: 炸弹不能通杀，只能炸2的牌型和三连对牌型
            // 普通单张/对子（非2）不能被炸，直接返回 false
            if (type1 == ToSouthFreeCardType.SINGLE) {
                return prev.getFirst().getRank() == RANK_2; // 只有单张2才能被炸
            }
            if (type1 == ToSouthFreeCardType.PAIR) {
                return prev.getFirst().getRank() == RANK_2; // 只有对子2才能被炸
            }
            // 炸三连对；四连对及以上不能被炸
            if (type1 == ToSouthFreeCardType.CONSECUTIVE_PAIRS) return prev.size() < 8;
            // 顺子、三张等其他牌型不能被炸
            return false;
        }

        // 连对: 只能大过 单2/对2/同类型
        if (type2 == ToSouthFreeCardType.CONSECUTIVE_PAIRS) {
            // 3连对(6张) > 单张2
            if (current.size() == 6 && type1 == ToSouthFreeCardType.SINGLE && prev.getFirst().getRank() == RANK_2) return true;
            // 四连对及以上可以炸掉：单个2、双22、3连对、4张
            if (current.size() >= 8) {
                if ((type1 == ToSouthFreeCardType.PAIR || type1 == ToSouthFreeCardType.SINGLE) && prev.getFirst().getRank() == RANK_2) return true;
                if (type1 == ToSouthFreeCardType.BOMB_QUAD) return true;
                // 4连对+ > 3连对(6张)
                if (type1 == ToSouthFreeCardType.CONSECUTIVE_PAIRS && prev.size() == 6) return true;
            }

            // 连对 vs 连对
            if (type1 == ToSouthFreeCardType.CONSECUTIVE_PAIRS && prev.size() == current.size()) {
                return compareMaxCard(prev, current);
            }
    ...
    private static boolean isBombType(ToSouthFreeCardType type) {
    ...
    public static List<List<Card>> findAllBestPlays(List<Card> handCards) {
        ...
        // 1. 整合牌型 (分解手牌)
        Map<ToSouthFreeCardType, List<List<Card>>> integratedCards = integrateHandCards(handCards);

        // 2. 按优先级查找
        // 优先级：顺子 > 三条 > 对子 > 单张 > 连对 > 炸弹

        // 2.4 顺子
        List<List<Card>> straights = integratedCards.get(ToSouthFreeCardType.STRAIGHT);
        if (CollUtil.isNotEmpty(straights)) result.addAll(straights);

        // 2.3 三张
        List<List<Card>> triples = integratedCards.get(ToSouthFreeCardType.TRIPLE);
        if (CollUtil.isNotEmpty(triples)) result.addAll(triples);

        // 2.2 对子
        List<List<Card>> pairs = integratedCards.get(ToSouthFreeCardType.PAIR);
        if (CollUtil.isNotEmpty(pairs)) result.addAll(pairs);

        // 2.1 单张
        List<List<Card>> singles = integratedCards.get(ToSouthFreeCardType.SINGLE);
        if (CollUtil.isNotEmpty(singles)) result.addAll(singles);

        // 2.5 连对
        List<List<Card>> consecutivePairs = integratedCards.get(ToSouthFreeCardType.CONSECUTIVE_PAIRS);
        if (CollUtil.isNotEmpty(consecutivePairs)) result.addAll(consecutivePairs);

        // 2.6 炸弹
        List<List<Card>> bombs = integratedCards.get(ToSouthFreeCardType.BOMB_QUAD);
        ...
        result.sort((list1, list2) -> {
            ...
            ToSouthFreeCardType t1 = getCardType(list1);
            ToSouthFreeCardType t2 = getCardType(list2);
            // 如果类型不同，按枚举顺序
    ...
    private static int getCardTypePriority(ToSouthFreeCardType type) {
        return switch (type) {
            case SINGLE -> 4;
            case PAIR -> 3;
            case TRIPLE -> 2;
    ...
    public static List<List<Card>> findAllFollowPlays(List<Card> handCards, List<Card> lastCards) {
        ...
        // 识别上家牌型
        ToSouthFreeCardType lastType = getCardType(lastCards);
        if (lastType == ToSouthFreeCardType.NONE) return Collections.emptyList();

        // 1. 尝试找同类型压制
        Map<ToSouthFreeCardType, List<List<Card>>> integratedCards = integrateHandCards(handCards);

        if (lastType == ToSouthFreeCardType.SINGLE) {
        ...
        } else if (lastType == ToSouthFreeCardType.PAIR) {
        ...
        } else if (lastType == ToSouthFreeCardType.TRIPLE) {
        ...
        } else if (lastType == ToSouthFreeCardType.STRAIGHT) {
        ...
        } else if (lastType == ToSouthFreeCardType.CONSECUTIVE_PAIRS) {
        ...
        if (CollUtil.isNotEmpty(consecutivePairs)) {
            if (lastType == ToSouthFreeCardType.SINGLE && lastCards.getFirst().getRank() == RANK_2) {
            ...
            } else if (lastType == ToSouthFreeCardType.PAIR && lastCards.getFirst().getRank() == RANK_2) {
                // 上家是对2，找4连对(8张)
                for (List<Card> cp : consecutivePairs) {
                    if (cp.size() >= 8) {
                        List<Card> sortedCp = new ArrayList<>(cp);
            ...
            } else if (lastType == ToSouthFreeCardType.BOMB_QUAD) {
            ...
            } else if (lastType == ToSouthFreeCardType.CONSECUTIVE_PAIRS && lastCards.size() == 6) {
        ...
        result.sort((list1, list2) -> {
            ...
            if (isBomb1) {
                ...
                boolean isConsec1 = (t1 == ToSouthFreeCardType.CONSECUTIVE_PAIRS);
                boolean isConsec2 = (t2 == ToSouthFreeCardType.CONSECUTIVE_PAIRS);
    ...
    public static List<Integer> sortAndGetHighlightCards(List<Card> cards) {
        ...
        Map<ToSouthFreeCardType, List<List<Card>>> integrated = integrateHandCards(cards);

        // 收集高亮牌: 炸弹(四条) + 连对炸弹 + 2
        List<List<Card>> bombs = integrated.get(ToSouthFreeCardType.BOMB_QUAD);
        if (CollUtil.isNotEmpty(bombs)) {
            for (List<Card> bomb : bombs) {
                for (Card c : bomb) {
        ...
        List<List<Card>> cps = integrated.get(ToSouthFreeCardType.CONSECUTIVE_PAIRS);
        ...
        addGroups(sorted, integrated.get(ToSouthFreeCardType.BOMB_QUAD));     // 2. 4张炸弹
        addGroups(sorted, threeCPs);                                      // 3. 3连对炸弹
        addGroups(sorted, integrated.get(ToSouthFreeCardType.STRAIGHT));      // 4. 顺子
        addGroups(sorted, integrated.get(ToSouthFreeCardType.TRIPLE));        // 5. 3张
        addGroups(sorted, integrated.get(ToSouthFreeCardType.PAIR));          // 6. 对子
        addGroups(sorted, integrated.get(ToSouthFreeCardType.SINGLE));        // 7. 单张
     ...
     * 排序优先级: 炸弹（4连对炸弹 > 4张炸弹 > 3连对炸弹）> 顺子 > 3张 > 对子 > 单张
     */
    public static Map<ToSouthFreeCardType, List<List<Card>>> integrateHandCards(List<Card> handCards) {
        Map<ToSouthFreeCardType, List<List<Card>>> result = new HashMap<>();
        // 复制并排序 (降序 2..3)
        ...
        result.put(ToSouthFreeCardType.BOMB_QUAD, bombs);

        // 3. 提取三连对炸弹（四条提取后，剩余count>=2的重新找链长>=3）
        extractPairChains(consecutivePairs, rankMap, 3);
        result.put(ToSouthFreeCardType.CONSECUTIVE_PAIRS, consecutivePairs);
        ...
        result.put(ToSouthFreeCardType.TRIPLE, triples);
        ...
        result.put(ToSouthFreeCardType.STRAIGHT, straights);
        ...
        result.put(ToSouthFreeCardType.PAIR, pairs);
        result.put(ToSouthFreeCardType.SINGLE, singles);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Map<ToSouthFreeCardType, List<List<Card>>> integrated = integrateHandCards(hand);
