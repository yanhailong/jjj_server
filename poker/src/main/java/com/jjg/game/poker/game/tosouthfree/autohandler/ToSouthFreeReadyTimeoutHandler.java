...
public class ToSouthFreeReadyTimeoutHandler implements IProcessorHandler {
    private static final Logger log = LoggerFactory.getLogger(ToSouthFreeReadyTimeoutHandler.class);
    ...
    public ToSouthFreeReadyTimeoutHandler(long playerId, long gameId, long timerVersion, AbstractRoomController<Room_ChessCfg, ? extends Room> roomController) {
    ...
    @Override
    public void action() {
        ToSouthFreeGameController gameController = (ToSouthFreeGameController) roomController.getGameController();
        if (gameController == null) {
            return;
        }
        if (gameController instanceof AbstractPhaseGameController<?, ?> phaseGameController) {
            ToSouthFreeGameDataVo gameDataVo = (ToSouthFreeGameDataVo) roomController.getGameController().getGameDataVo();
