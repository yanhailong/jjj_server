...
import static com.jjg.game.poker.game.tosouthblood.constant.ToSouthBloodConstant.InstantWinType.*;

/**
 * 南方前进-血战牌型工具类
 */
public class ToSouthBloodHandUtils {
    ...
    private static final Logger log = LoggerFactory.getLogger(ToSouthBloodHandUtils.class);
     ...
     * 判断牌型
     */
    public static ToSouthBloodCardType getCardType(List<Card> cards) {
        if (cards == null || cards.isEmpty()) return ToSouthBloodCardType.NONE;
        int size = cards.size();
        cards.sort(CARD_COMPARATOR);

        if (size == 1) return ToSouthBloodCardType.SINGLE;
        if (size == 2 && isPair(cards)) return ToSouthBloodCardType.PAIR;
        if (size == 3 && isTriple(cards)) return ToSouthBloodCardType.TRIPLE;
        if (size == 4 && isBombQuad(cards)) return ToSouthBloodCardType.BOMB_QUAD; // 四张是炸弹
        if (size >= 3 && isStraight(cards)) return ToSouthBloodCardType.STRAIGHT;
        if (size >= 6 && size % 2 == 0 && isConsecutivePairs(cards)) return ToSouthBloodCardType.CONSECUTIVE_PAIRS;

        return ToSouthBloodCardType.NONE;
    }
    ...
    public static boolean compare(List<Card> prev, List<Card> current) {
        ToSouthBloodCardType type1 = getCardType(prev);
        ToSouthBloodCardType type2 = getCardType(current);
        if (type2 == ToSouthBloodCardType.NONE) return false;
        // 炸弹牌型，只能炸2的牌型和三连对，不能炸顺子、单张对子3-A等普通牌型
        if (type2 == ToSouthBloodCardType.BOMB_QUAD) {
            if (type1 == ToSouthBloodCardType.BOMB_QUAD) {
                // 都是四张，比大小
                return compareMaxCard(prev, current);
            }
            // 规则: 炸弹不能通杀，只能炸2的牌型和三连对牌型
            // 普通单张/对子（非2）不能被炸，直接返回 false
            if (type1 == ToSouthBloodCardType.SINGLE) {
                return prev.getFirst().getRank() == RANK_2; // 只有单张2才能被炸
            }
            if (type1 == ToSouthBloodCardType.PAIR) {
                return prev.getFirst().getRank() == RANK_2; // 只有对子2才能被炸
            }
            // 炸三连对；四连对及以上不能被炸
            if (type1 == ToSouthBloodCardType.CONSECUTIVE_PAIRS) return prev.size() < 8;
            // 顺子、三张等其他牌型不能被炸
            return false;
        }

        // 连对: 只能大过 单2/对2/同类型
        if (type2 == ToSouthBloodCardType.CONSECUTIVE_PAIRS) {
            // 3连对(6张) > 单张2
            if (current.size() == 6 && type1 == ToSouthBloodCardType.SINGLE && prev.getFirst().getRank() == RANK_2) return true;
            // 四连对及以上可以炸掉：单个2、双22、3连对、4张
            if (current.size() >= 8) {
                if ((type1 == ToSouthBloodCardType.PAIR || type1 == ToSouthBloodCardType.SINGLE) && prev.getFirst().getRank() == RANK_2) return true;
                if (type1 == ToSouthBloodCardType.BOMB_QUAD) return true;
                // 4连对+ > 3连对(6张)
                if (type1 == ToSouthBloodCardType.CONSECUTIVE_PAIRS && prev.size() == 6) return true;
            }

            // 连对 vs 连对
            if (type1 == ToSouthBloodCardType.CONSECUTIVE_PAIRS && prev.size() == current.size()) {
                return compareMaxCard(prev, current);
            }
    ...
    private static boolean isBombType(ToSouthBloodCardType type) {
    ...
    public static List<List<Card>> findAllBestPlays(List<Card> handCards) {
        ...
        // 1. 整合牌型 (分解手牌)
        Map<ToSouthBloodCardType, List<List<Card>>> integratedCards = integrateHandCards(handCards);

        // 2. 按优先级查找
        // 优先级：顺子 > 三条 > 对子 > 单张 > 连对 > 炸弹

        // 2.4 顺子
        List<List<Card>> straights = integratedCards.get(ToSouthBloodCardType.STRAIGHT);
        if (CollUtil.isNotEmpty(straights)) result.addAll(straights);

        // 2.3 三张
        List<List<Card>> triples = integratedCards.get(ToSouthBloodCardType.TRIPLE);
        if (CollUtil.isNotEmpty(triples)) result.addAll(triples);

        // 2.2 对子
        List<List<Card>> pairs = integratedCards.get(ToSouthBloodCardType.PAIR);
        if (CollUtil.isNotEmpty(pairs)) result.addAll(pairs);

        // 2.1 单张
        List<List<Card>> singles = integratedCards.get(ToSouthBloodCardType.SINGLE);
        if (CollUtil.isNotEmpty(singles)) result.addAll(singles);

        // 2.5 连对
        List<List<Card>> consecutivePairs = integratedCards.get(ToSouthBloodCardType.CONSECUTIVE_PAIRS);
        if (CollUtil.isNotEmpty(consecutivePairs)) result.addAll(consecutivePairs);

        // 2.6 炸弹
        List<List<Card>> bombs = integratedCards.get(ToSouthBloodCardType.BOMB_QUAD);
        ...
        result.sort((list1, list2) -> {
            ...
            ToSouthBloodCardType t1 = getCardType(list1);
            ToSouthBloodCardType t2 = getCardType(list2);
            // 如果类型不同，按枚举顺序
    ...
    private static int getCardTypePriority(ToSouthBloodCardType type) {
        return switch (type) {
            case SINGLE -> 4;
            case PAIR -> 3;
            case TRIPLE -> 2;
    ...
    public static List<List<Card>> findAllFollowPlays(List<Card> handCards, List<Card> lastCards) {
        ...
        // 识别上家牌型
        ToSouthBloodCardType lastType = getCardType(lastCards);
        if (lastType == ToSouthBloodCardType.NONE) return Collections.emptyList();

        // 1. 尝试找同类型压制
        Map<ToSouthBloodCardType, List<List<Card>>> integratedCards = integrateHandCards(handCards);

        if (lastType == ToSouthBloodCardType.SINGLE) {
        ...
        } else if (lastType == ToSouthBloodCardType.PAIR) {
        ...
        } else if (lastType == ToSouthBloodCardType.TRIPLE) {
        ...
        } else if (lastType == ToSouthBloodCardType.STRAIGHT) {
        ...
        } else if (lastType == ToSouthBloodCardType.CONSECUTIVE_PAIRS) {
        ...
        if (CollUtil.isNotEmpty(consecutivePairs)) {
            if (lastType == ToSouthBloodCardType.SINGLE && lastCards.getFirst().getRank() == RANK_2) {
            ...
            } else if (lastType == ToSouthBloodCardType.PAIR && lastCards.getFirst().getRank() == RANK_2) {
                // 上家是对2，找4连对(8张)
                for (List<Card> cp : consecutivePairs) {
                    if (cp.size() >= 8) {
                        List<Card> sortedCp = new ArrayList<>(cp);
            ...
            } else if (lastType == ToSouthBloodCardType.BOMB_QUAD) {
            ...
            } else if (lastType == ToSouthBloodCardType.CONSECUTIVE_PAIRS && lastCards.size() == 6) {
        ...
        result.sort((list1, list2) -> {
            ...
            if (isBomb1) {
                ...
                boolean isConsec1 = (t1 == ToSouthBloodCardType.CONSECUTIVE_PAIRS);
                boolean isConsec2 = (t2 == ToSouthBloodCardType.CONSECUTIVE_PAIRS);
    ...
    public static List<Integer> sortAndGetHighlightCards(List<Card> cards) {
        ...
        Map<ToSouthBloodCardType, List<List<Card>>> integrated = integrateHandCards(cards);

        // 收集高亮牌: 炸弹(四条) + 连对炸弹 + 2
        List<List<Card>> bombs = integrated.get(ToSouthBloodCardType.BOMB_QUAD);
        if (CollUtil.isNotEmpty(bombs)) {
            for (List<Card> bomb : bombs) {
                for (Card c : bomb) {
        ...
        List<List<Card>> cps = integrated.get(ToSouthBloodCardType.CONSECUTIVE_PAIRS);
        ...
        addGroups(sorted, integrated.get(ToSouthBloodCardType.BOMB_QUAD));     // 2. 4张炸弹
        addGroups(sorted, threeCPs);                                      // 3. 3连对炸弹
        addGroups(sorted, integrated.get(ToSouthBloodCardType.STRAIGHT));      // 4. 顺子
        addGroups(sorted, integrated.get(ToSouthBloodCardType.TRIPLE));        // 5. 3张
        addGroups(sorted, integrated.get(ToSouthBloodCardType.PAIR));          // 6. 对子
        addGroups(sorted, integrated.get(ToSouthBloodCardType.SINGLE));        // 7. 单张
     ...
     * 排序优先级: 炸弹（4连对炸弹 > 4张炸弹 > 3连对炸弹）> 顺子 > 3张 > 对子 > 单张
     */
    public static Map<ToSouthBloodCardType, List<List<Card>>> integrateHandCards(List<Card> handCards) {
        Map<ToSouthBloodCardType, List<List<Card>>> result = new HashMap<>();
        // 复制并排序 (降序 2..3)
        ...
        result.put(ToSouthBloodCardType.BOMB_QUAD, bombs);

        // 3. 提取三连对炸弹（四条提取后，剩余count>=2的重新找链长>=3）
        extractPairChains(consecutivePairs, rankMap, 3);
        result.put(ToSouthBloodCardType.CONSECUTIVE_PAIRS, consecutivePairs);
        ...
        result.put(ToSouthBloodCardType.TRIPLE, triples);
        ...
        result.put(ToSouthBloodCardType.STRAIGHT, straights);
        ...
        result.put(ToSouthBloodCardType.PAIR, pairs);
        result.put(ToSouthBloodCardType.SINGLE, singles);
    ...
    public static void main(String[] args) {
        System.out.println("=== 南方前进-血战牌型工具类测试 ===");
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Map<ToSouthBloodCardType, List<List<Card>>> integrated = integrateHandCards(hand);
