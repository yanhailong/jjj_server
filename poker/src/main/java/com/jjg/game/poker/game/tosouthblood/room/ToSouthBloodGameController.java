...
import com.jjg.game.poker.game.tosouthblood.autohandler.ToSouthBloodAutoPlayHandler;
import com.jjg.game.poker.game.tosouthblood.autohandler.ToSouthBloodReadyTimeoutHandler;
import com.jjg.game.poker.game.tosouthblood.autohandler.ToSouthBloodRobotHandler;
...
import com.jjg.game.poker.game.tosouthblood.gamephase.ToSouthBloodSettlementPhase;
import com.jjg.game.poker.game.tosouthblood.gamephase.ToSouthBloodStartGamePhase;
import com.jjg.game.poker.game.tosouthblood.message.bean.*;
import com.jjg.game.poker.game.tosouthblood.message.notify.NotifyToSouthBloodBombSettlement;
import com.jjg.game.poker.game.tosouthblood.message.notify.NotifyToSouthBloodPlayerReady;
import com.jjg.game.poker.game.tosouthblood.message.notify.NotifyToSouthBloodTurnActionInfo;
...
import com.jjg.game.poker.game.tosouthblood.message.resp.RespToSouthBloodChangTable;
import com.jjg.game.poker.game.tosouthblood.message.resp.RespToSouthBloodRoomBaseInfo;
...
import com.jjg.game.poker.game.tosouthblood.room.data.ToSouthBloodRoundRecord;
...
@GameController(gameType = EGameType.TO_SOUTH_BLOOD, roomType = RoomType.POKER_ROOM)
public class ToSouthBloodGameController extends BasePokerGameController<ToSouthBloodGameDataVo> {
    /**
     * 准备倒计时（毫秒）
     */
    private static final int READY_TIMEOUT = 11000;

    public ToSouthBloodGameController(AbstractRoomController<Room_ChessCfg, ? extends Room> roomController) {
     ...
     * 换桌
     */
    public void reqChangeTable(PlayerController playerController, ToSouthBloodGameController controller) {
        ...
        RespToSouthBloodChangTable res = new RespToSouthBloodChangTable(changed ? Code.SUCCESS : Code.FAIL);
    ...
    public void turnAction(long playerId, ReqTurnAction reqTurnAction) {
        ...
        if (log.isDebugEnabled()) {
            ...
            if (actionType == 0 && CollUtil.isNotEmpty(reqTurnAction.cards)) {
                ...
                c.sort(ToSouthBloodHandUtils.CARD_COMPARATOR);
                cardStr = ToSouthBloodHandUtils.cardListToString(c);
            }
        ...
        // 2. 牌型检查
        ToSouthBloodCardType type = ToSouthBloodHandUtils.getCardType(playCards);
        if (type == ToSouthBloodCardType.NONE) {
            log.warn("[南方前进-血战][出牌] 非法牌型 - 玩家: {}, 座位: {}, 牌: {}",
                    playerId, info.getSeatId(), ToSouthBloodHandUtils.cardListToString(playCards));
            return;
        }
        log.info("[南方前进-血战][出牌] 玩家: {}, 座位: {}, 牌型: {}, 牌: {}",
                playerId, info.getSeatId(), type, ToSouthBloodHandUtils.cardListToString(playCards));
        ...
        if (gameDataVo.getRoundLeaderSeatId() != info.getSeatId()) {
            ...
            log.info("[南方前进-血战][比牌] 上家牌型: {}, 上家牌: {} | 当前牌型: {}, 当前牌: {}",
                    lastType, ToSouthBloodHandUtils.cardListToString(lastCards),
                    type, ToSouthBloodHandUtils.cardListToString(playCards));
            if (!ToSouthBloodHandUtils.compare(lastCards, playCards)) {
                log.warn("[南方前进-血战][比牌] 管不上 - 玩家: {}, {} [{}] 无法压过 {} [{}]",
                        playerId,
                        ToSouthBloodHandUtils.cardListToString(playCards), type,
                        ToSouthBloodHandUtils.cardListToString(lastCards), lastType);
                return;
            }
            log.info("[南方前进-血战][比牌] 管牌成功 - 玩家: {}, {} [{}] 压过 {} [{}]",
                    playerId,
                    ToSouthBloodHandUtils.cardListToString(playCards), type,
                    ToSouthBloodHandUtils.cardListToString(lastCards), lastType);
        } else {
            log.info("[南方前进-血战][首出] 玩家: {}, 座位: {}, 牌型: {}, 牌: {}",
                    playerId, info.getSeatId(), type, ToSouthBloodHandUtils.cardListToString(playCards));
        }
        ...
        if (!info.getCurrentCards().isEmpty()) {
            ...
            List<Integer> highlightIds = ToSouthBloodHandUtils.sortAndGetHighlightCards(remainingCards);
        ...
        // 记录出牌
        gameDataVo.getCurrentRoundPlays().add(new ToSouthBloodRoundRecord(info.getSeatId(), realPlayCardIds, playCardIds, type));
        // 记录出牌到一局日志（区分 首出 / 跟牌 / 新一轮出牌）
        playCards.sort(ToSouthBloodHandUtils.CARD_COMPARATOR);
        String cardsStr = ToSouthBloodHandUtils.cardListToString(playCards);
    ...
    private void processBombSettlement(int winnerSeatId) {
        List<ToSouthBloodRoundRecord> plays = gameDataVo.getCurrentRoundPlays();
        if (CollUtil.isEmpty(plays)) return;

        ToSouthBloodRoundRecord lastPlay = plays.getLast();
        ...
        // 炸弹链
        List<ToSouthBloodRoundRecord> bombChain = new ArrayList<>();
        int victimIndex = -1;
        ...
        for (int i = plays.size() - 1; i >= 0; i--) {
            ToSouthBloodRoundRecord record = plays.get(i);
        ...
        // 打印扫描结果
        if (log.isDebugEnabled()) {
            StringBuilder sb = new StringBuilder("[南方前进-血战][炸弹结算] 扫描结果 - victimIndex=")
                    .append(victimIndex).append(", bombChain(共").append(bombChain.size()).append("条):");
            for (int i = 0; i < bombChain.size(); i++) {
                ToSouthBloodRoundRecord r = bombChain.get(i);
                sb.append("\n    [").append(i).append("] seat=").append(r.seatId)
                        .append(" type=").append(r.cardType);
            }
            if (victimIndex >= 0) {
                ToSouthBloodRoundRecord v = plays.get(victimIndex);
        ...
        ToSouthBloodRoundRecord winnerRecord = bombChain.getLast();
        ...
        List<ToSouthBloodBombDetail> details = new ArrayList<>();

        // 守卫：如果被炸的牌不是可炸牌型（非2单张/对子、非炸弹、非连对），不触发结算
        // 普通单张（非2）、对子（非2）、三张、顺子等均不能被炸
        if (victimIndex != -1) {
            ToSouthBloodRoundRecord victimRecord = plays.get(victimIndex);
            List<Card> victimCards = playCardsIdsToCards(victimRecord.cards, cardMap);
            victimCards.sort(ToSouthBloodHandUtils.CARD_COMPARATOR);
        ...
        List<ToSouthBloodRoundRecord> settledRecords = new ArrayList<>();
        if (victimIndex != -1) {
        ...
        if (CollUtil.isNotEmpty(settledRecords)) {
            ...
            for (ToSouthBloodRoundRecord settledRecord : settledRecords) {
                ...
                log.debug("[南方前进-血战][炸弹结算] seat={} type={} 倍数={}",
            ...
            int detailType;
            if (bombChain.size() >= 2) {
                ToSouthBloodRoundRecord secondLast = bombChain.get(bombChain.size() - 2);
                ...
                ToSouthBloodRoundRecord directVictim = victimIndex >= 0 ? plays.get(victimIndex) : settledRecords.getFirst();
            ...
            if (score > 0 && victimId != winnerId) {
                ...
                long winScore = 0;
                for (ToSouthBloodBombDetail d : details) {
                    if (d.playerId == winnerId && d.type == ToSouthBloodConstant.BOMB_WIN_TYPE) {
                        winScore = d.score;
                    }
                }
        ...
        if (CollUtil.isNotEmpty(details)) {
            NotifyToSouthBloodBombSettlement notify = new NotifyToSouthBloodBombSettlement();
            notify.details = details;
            broadcastToPlayers(RoomMessageBuilder.newBuilder().sendAllPlayer(notify));
        }
    }

    private void addBombScore(List<ToSouthBloodBombDetail> details, long loserId, long winnerId, long score, int type) {
        // 直接扣除输家积分
        // 炸弹扣钱
        deductItem(loserId, score, AddType.GAME_SETTLEMENT, "ToSouthBlood bomb loses money", false);
        details.add(new ToSouthBloodBombDetail(loserId, score, ToSouthBloodConstant.BOMB_LOSE_TYPE));
        ...
        details.add(new ToSouthBloodBombDetail(winnerId, finalWinScore, ToSouthBloodConstant.BOMB_WIN_TYPE));
    }
     ...
     */
    private long getBombSettlementMultiplier(ToSouthBloodRoundRecord victimRecord,
        ...
        if (allRank2 && (victimRecord.cardType == ToSouthBloodCardType.SINGLE
                || victimRecord.cardType == ToSouthBloodCardType.PAIR
                || victimRecord.cardType == ToSouthBloodCardType.TRIPLE)) {
            long multi = 0;
        ...
        if (victimRecord.cardType == ToSouthBloodCardType.BOMB_QUAD) {
            return moneyCfg.getFourkindboom();
        }
        if (victimRecord.cardType == ToSouthBloodCardType.CONSECUTIVE_PAIRS) {
    ...
    private boolean isBomb(ToSouthBloodCardType type) {
...
//        actionInfo.canPlay = false;
        ...
        for (PlayerSeatInfo info : gameDataVo.getPlayerSeatInfoList()) {
            ...
            // 但为了绝对安全，这里使用 clone
            ToSouthBloodActionInfo playerActionInfo = cloneActionInfo(actionInfo);
        ...
        ToSouthBloodActionInfo waitPlayerActionInfo = cloneActionInfo(actionInfo);
    ...
    // 本轮出牌历史
    private void fillCurRoundPlayedCardsHistory(ToSouthBloodActionInfo actionInfo) {
        ...
        actionInfo.curRoundPlayedCardHistory = gameDataVo.getCurrentRoundPlays().stream().map(play -> {
            ToSouthBloodPlayCardRecord playCardRecord = new ToSouthBloodPlayCardRecord();
     ...
     */
    private void fillCommonActionInfo(ToSouthBloodActionInfo actionInfo) {
     ...
     * @param waitPlayer
     */
    private void fillRecommendCards(ToSouthBloodActionInfo actionInfo, PlayerSeatInfo waitPlayer) {
        if (waitPlayer == null) return;

        List<ToSouthBloodRecommendCards> recommendCardsList = new ArrayList<>();
        Map<Integer, PokerCard> cardMap = ToSouthBloodDataHelper.getCardListMap(ToSouthBloodDataHelper.getPoolId(gameDataVo));
        ...
        if (isFirstPlayer(waitPlayer.getSeatId())) {
            ...
            if (gameDataVo.isFirstRound()) {
                ...
                List<Card> best = handCards.stream()
                        ...
                        .findFirst()
                        .map(spade3 -> ToSouthBloodHandUtils.findBestPlayWithFirstCard(rankMap, spade3))
                        .orElse(null);
                if (CollUtil.isNotEmpty(best)) {
                    List<Integer> ids = ToSouthBloodDataHelper.getClientId(gameDataVo, best.stream().map(c -> ((PokerCard) c).getPokerPoolId()).collect(Collectors.toList()));
                    recommendCardsList.add(new ToSouthBloodRecommendCards(ids));
                }
            } else {
                // 普通首出 (获取所有可能)
                List<List<Card>> allBestPlays = ToSouthBloodHandUtils.findAllBestPlays(handCards);
                for (List<Card> play : allBestPlays) {
                    List<Integer> ids = ToSouthBloodDataHelper.getClientId(gameDataVo, play.stream().map(c -> ((PokerCard) c).getPokerPoolId()).collect(Collectors.toList()));
                    recommendCardsList.add(new ToSouthBloodRecommendCards(ids));
                }
            }
        } else {
            ...
            if (CollUtil.isNotEmpty(lastIds)) {
                ...
                List<List<Card>> allPlays = ToSouthBloodHandUtils.findAllFollowPlays(handCards, lastCards);
    ...
    private ToSouthBloodActionInfo cloneActionInfo(ToSouthBloodActionInfo source) {
        ToSouthBloodActionInfo target = new ToSouthBloodActionInfo();
    ...
    public void respRoomInitInfoAction(PlayerController playerController) {
        log.debug("响应南方前进-血战房间信息 - 玩家: {}", playerController.playerId());
        RespToSouthBloodRoomBaseInfo baseInfo = new RespToSouthBloodRoomBaseInfo(Code.SUCCESS);
    ...
    /**
     * 南方前进-血战请求准备/取消准备（在 WAIT_READY 阶段，四人全部准备后才开始发牌）
     *
     * @param playerId 玩家id
     * @param req      请求（status: 1=准备, 2=取消）
     */
    public void reqToSouthBloodGoReady(long playerId, ReqToSouthBloodGoReady req) {
        NotifyToSouthBloodPlayerReady notify = new NotifyToSouthBloodPlayerReady();
    ...
    public boolean tryStartGame() {
        ...
        addPokerPhaseTimer(new ToSouthBloodStartGamePhase(this, gameDataVo.getId()));
    ...
    public void scheduleReadyTimeout(long playerId, long timeOut) {
        ...
        ToSouthBloodReadyTimeoutHandler handler = new ToSouthBloodReadyTimeoutHandler(playerId, gameDataVo.getId(), exeTime, roomController);
    ...
    /**
     * 机器人准备（由 ToSouthBloodRobotHandler.GO_READY 回调）
     *
    ...
    public void robotGoReady(long playerId) {
        ...
        log.info("机器人 {} 准备完成，广播 NotifyToSouthBloodPlayerReady(playerId={}, status=1)，当前准备人数: {}/{}", playerId, playerId, readyCount, seatDownCount);
        tryStartGame();
    }
...
//            getRoomController().getRoomManager().exitRoom(playerId);
    ...
    public void onRunGamePlayerLeaveRoom(SeatInfo remove) {
        // 自动走 ToSouthBloodAutoPlayHandler 模块
    }
    ...
    public void addNextTimer(PlayerSeatInfo nextExePlayer, int sendCardNum) {
        ...
        if (gamePlayer instanceof GameRobotPlayer robotPlayer) {
            ...
            ToSouthBloodAutoPlayHandler handler = new ToSouthBloodAutoPlayHandler(playerId, gameDataVo.getId(), this);
            addPlayerTimer(handler, delay);
        } else {
            // 真实玩家，添加超时自动操作
            addPlayerTimer(new ToSouthBloodAutoPlayHandler(playerId, gameDataVo.getId(), this), operationTime);
        }
    }
    ...
    @Override
    protected ToSouthBloodGameDataVo createRoomDataVo(Room_ChessCfg roomCfg) {
        return new ToSouthBloodGameDataVo(roomCfg);
    }

    @Override
    public EGameType gameControlType() {
        return EGameType.TO_SOUTH_BLOOD;
    }
}
