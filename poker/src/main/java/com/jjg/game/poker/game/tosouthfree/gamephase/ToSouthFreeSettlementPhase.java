 ...
 * 南方前进-免费结算阶段
 */
public class ToSouthFreeSettlementPhase extends BaseSettlementPhase<ToSouthFreeGameDataVo> {
    private static final Logger log = LoggerFactory.getLogger(ToSouthFreeSettlementPhase.class);
    private final ToSouthFreeSettlementContext context;
    private final List<PlayerSeatInfo> winners;

    public ToSouthFreeSettlementPhase(AbstractPhaseGameController<Room_ChessCfg, ToSouthFreeGameDataVo> gameController, ToSouthFreeSettlementContext context) {
    ...
    public void phaseFinishDoAction() {
        ...
        for (GamePlayer gamePlayer : players) {
            ...
            if (roomPlayer != null && !roomPlayer.isOnline()) {
                log.info("南方前进-免费结算后：玩家 {} 离线，踢出房间", playerId);
    ...
    public void phaseDoAction() {
        ...
        if (gameController instanceof ToSouthFreeGameController controller) {
            ...
            // 应用结算结果
            long totalTax = 0;
            List<ToSouthFreePlayerSettlementInfo> playerSettlementInfos = new ArrayList<>();
...
//                                26000 - 8000
            ...
            for (Map.Entry<Long, Long> entry : settlementMap2.entrySet()) {
                ...
                GamePlayer gamePlayer = gameDataVo.getGamePlayer(playerId);
                if (gamePlayer == null) {
                    log.error("南方前进-免费结算时 gamePlayer=null playerId:{}", playerId);
                    continue;
                }
                ...
                if (change > 0) {
                    ...
                    if (loseAmount > 0) {
                        //南方前进-免费输钱
                        controller.deductItem(playerId, loseAmount, AddType.GAME_SETTLEMENT, "ToSouthFree loses money", false);
                    }
                ...
                // 构建玩家结算信息
                ToSouthFreePlayerSettlementInfo info = new ToSouthFreePlayerSettlementInfo();
                ...
                if (seatInfo != null) {
                    ...
                    if (context.isInstantWin()) {
                        for (ToSouthFreeSettlementContext.SettlementItem item : context.getSettlementItems()) {
            ...
            ToSouthFreeGameLog gameLog = gameDataVo.getGameLog();
            Map<Integer, PokerCard> cardMapForLog = ToSouthFreeDataHelper.getCardListMap(ToSouthFreeDataHelper.getPoolId(gameDataVo));
            for (ToSouthFreePlayerSettlementInfo sInfo : playerSettlementInfos) {
                // 计算剩余手牌数
                ...
                if (!sInfo.isWinner && !context.isInstantWin() && seat != null) {
                    ...
                    int countTwo = ToSouthFreeHandUtils.countTwo(handCards);
                    int countRedTwo = ToSouthFreeHandUtils.countRedTwo(handCards);
                    ...
                    int optimalBombMulti = ToSouthFreeHandUtils.calcOptimalBombMultiplier(
    ...
    private long getBaseBet(ToSouthFreeGameDataVo gameDataVo) {
        return gameDataVo.getRoomBet(); // Placeholder
    }

    private void calSettlement(ToSouthFreeGameDataVo gameDataVo, Map<Long, Long> settlementMap, long baseBet, SouthernMoneyCfg moneyCfg) {
        ...
        for (PlayerSeatInfo loser : losers) {
            ...
            int cardCount = handCards.size();
            handCards.sort(ToSouthFreeHandUtils.CARD_COMPARATOR);
            log.debug("计算输家 {} 分数 - 剩余手牌: {}", loser.getPlayerId(), ToSouthFreeHandUtils.cardListToString(handCards));
     ...
     * 同时持久化到Redis（跨房间保留）
     */
    private void updatePlayerWinStreak(ToSouthFreeGameDataVo gameDataVo, Map<Long, Long> settlementMap2) {
        Map<Long, Integer> streakMap = gameDataVo.getPlayerWinStreakMap();
        Map<Long, Long> profitMap = gameDataVo.getPlayerTotalProfitMap();

        ToSouthFreeCardLibManager cardLibManager = null;
        try {
            cardLibManager = CommonUtil.getContext().getBean(ToSouthFreeCardLibManager.class);
        } catch (Exception e) {
            log.error("获取ToSouthFreeCardLibManager异常", e);
        }
     ...
     */
    private void updatePoolBalance(ToSouthFreeGameDataVo gameDataVo, Map<Long, Long> settlementMap2) {
        if (!(gameController instanceof ToSouthFreeGameController controller)) {
            return;
        }
        ToSouthFreeCardLibManager cardLibManager = null;
        try {
            ...
            log.error("获取ToSouthFreeCardLibManager异常，跳过水池更新", e);
            return;
        }
     ...
     */
    private void addLog(ToSouthFreeGameController controller, ToSouthFreeGameDataVo gameDataVo,
                        Map<Long, Long> settlementMap2, List<ToSouthFreePlayerSettlementInfo> playerSettlementInfos) {
        // 1. 游戏级数据：完整流程事件列表（结构化对象，FastJSON 会序列化为 JSON 数组）
        gameDataTracker.addGameLogData("toSouthFreeInfo", gameDataVo.getGameLog().getEvents());

        // 2. 玩家级数据
        // 从结算信息中构建 playerId -> finalWinScore 映射
        Map<Long, Long> winScoreMap = new HashMap<>();
        for (ToSouthFreePlayerSettlementInfo info : playerSettlementInfos) {
     ...
     * 好友房：计算房主收益并记录流水
     */
    private void addCreateRecord(ToSouthFreeGameController controller, long totalTax, Map<Long, Long> settlementMap2) {
