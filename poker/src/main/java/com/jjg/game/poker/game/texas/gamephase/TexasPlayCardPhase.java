...
public class TexasPlayCardPhase extends BasePlayCardPhase<TexasGameDataVo> {
    ...
    private Pair<Long, Long> BBAndSBBet(TexasGameController controller, TexasCfg texasCfg) {
        ...
        if (!(bBGamePlayer instanceof GameRobotPlayer)) {
            gameController.triggerSettlementAction(bBGamePlayer.getId(), gameController.gameControlType().getGameTypeId(),
                    BBBetValue, BBBetValue, 0, gameController.getGameTransactionItemId());
        }
        if (!(sBGamePlayer instanceof GameRobotPlayer)) {
            gameController.triggerSettlementAction(sBGamePlayer.getId(), gameController.gameControlType().getGameTypeId(),
                    sBBetValue, sBBetValue, 0, gameController.getGameTransactionItemId());
        }
