 ...
 *     <li>单局模拟委托给 {@link ToSouthBloodCardLibGenerator}</li>
 *     <li>玩家连胜/盈亏的委托方法</li>
 * </ul>
 */
@Component
public class ToSouthBloodCardLibManager
        extends AbstractCardLibManager<ToSouthBloodCardLib, ToSouthBloodCardLibDao> {

    /** 南方前进游戏 ID */
    private static final int GAME_TYPE_TO_SOUTH = 300400;

    @Autowired
    private ToSouthBloodCardLibDao cardLibDao;
    ...
    @Override
    protected ToSouthBloodCardLibDao getCardLibDao() {
        return cardLibDao;
    }
    ...
    @Override
    protected List<ToSouthBloodCardLib> simulateOneGame() {
        if (cachedMoneyCfg == null) {
            return Collections.emptyList();
        }
        return ToSouthBloodCardLibGenerator.simulateOneGame(cachedPoolId, cachedMoneyCfg);
    }
