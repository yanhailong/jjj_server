 ...
 */
public class ToSouthBloodReadyTimeoutHandler implements IProcessorHandler {
    private static final Logger log = LoggerFactory.getLogger(ToSouthBloodReadyTimeoutHandler.class);
    ...
    public ToSouthBloodReadyTimeoutHandler(long playerId, long gameId, long timerVersion, AbstractRoomController<Room_ChessCfg, ? extends Room> roomController) {
    ...
    @Override
    public void action() {
        ToSouthBloodGameController gameController = (ToSouthBloodGameController) roomController.getGameController();
        if (gameController == null) {
            return;
        }
        if (gameController instanceof AbstractPhaseGameController<?, ?> phaseGameController) {
            ToSouthBloodGameDataVo gameDataVo = (ToSouthBloodGameDataVo) roomController.getGameController().getGameDataVo();
