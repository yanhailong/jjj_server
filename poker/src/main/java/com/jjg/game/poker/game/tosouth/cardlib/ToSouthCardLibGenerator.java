 ...
 */
public class ToSouthCardLibGenerator {
    private static final Logger log = LoggerFactory.getLogger(ToSouthCardLibGenerator.class);
     ...
     * @param poolId    牌池ID
     * @param moneyCfg  金钱配置
     * @return 2条ToSouthCardLib，可能为空（模拟异常时）
     */
    public static List<ToSouthCardLib> simulateOneGame(int poolId, SouthernMoneyCfg moneyCfg) {
        List<ToSouthCardLib> result = new ArrayList<>();
        ...
        for (int i = 0; i < SEAT_COUNT; i++) {
            ...
            Pair<Integer, List<Integer>> instantWin = ToSouthHandUtils.getInstantWinCards(handCards);
            if (instantWin != null) {
                hasInstantWin = true;
                break;
            }
    ...
    private static void simulatePlayAndSettle(List<List<Integer>> seatCards,
            ...
            if (isLeader) {
                ...
                if (isFirstRound) {
                    ...
                    if (spade3 != null) {
                        ...
                        playCards = ToSouthHandUtils.findBestPlayWithFirstCard(rankMap, spade3);
                    }
                }
                if (playCards == null) {
                    playCards = ToSouthHandUtils.findBestPlay(handCards);
                }
                ...
                if (lastPlayCardIds != null) {
                    ...
                    playCards = ToSouthHandUtils.findBestFollowPlay(handCards, lastCards);
                }
            }

            if (CollUtil.isNotEmpty(playCards)) {
                // 出牌成功
                ToSouthCardType cardType = ToSouthHandUtils.getCardType(playCards);
        ...
        // 计算最终结算倍数（与ToSouthSettlementPhase.calSettlement一致）
        ...
        for (int loserSeat : loserSeats) {
            ...
            int countTwo = ToSouthHandUtils.countTwo(loserHandCards);
            int countRedTwo = ToSouthHandUtils.countRedTwo(loserHandCards);
    ...
    /**
     * 模拟炸弹结算（简化版，复用ToSouthGameController的核心逻辑）
     */
    ...
    /**
     * 计算被炸牌型的赔付倍数（与ToSouthGameController.getBombSettlementMultiplier一致）
     */
    private static long getBombMultiplier(SimRoundRecord record,
        ...
        cards.sort(ToSouthHandUtils.CARD_COMPARATOR);

        boolean allRank2 = cards.stream().allMatch(c -> c.getRank() == RANK_2);
        if (allRank2 && (record.cardType == ToSouthCardType.SINGLE
                || record.cardType == ToSouthCardType.PAIR
                || record.cardType == ToSouthCardType.TRIPLE)) {
            long multi = 0;
            for (Card c : cards) {
        ...
        if (record.cardType == ToSouthCardType.BOMB_QUAD) {
            return moneyCfg.getFourkindboom();
        }
        if (record.cardType == ToSouthCardType.CONSECUTIVE_PAIRS) {
    ...
    private static boolean isBomb(ToSouthCardType type) {
     ...
     * 构建牌库条目
     */
    private static ToSouthCardLib buildCardLib(long multiplier, int targetSeat,
        ...
        return new ToSouthCardLib(multiplier, playerCards, robotCards);
    }
    ...
    private static class SimRoundRecord {
        int seatId;
        ToSouthCardType cardType;
        List<Integer> cards; // pokerPoolId

        SimRoundRecord(int seatId, ToSouthCardType cardType, List<Integer> cards) {
            this.seatId = seatId;
