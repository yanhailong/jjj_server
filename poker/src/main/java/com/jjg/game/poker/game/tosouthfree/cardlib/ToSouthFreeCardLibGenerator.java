 ...
 */
public class ToSouthFreeCardLibGenerator {
    private static final Logger log = LoggerFactory.getLogger(ToSouthFreeCardLibGenerator.class);
     ...
     * @param poolId    牌池ID
     * @param moneyCfg  金钱配置
     * @return 2条ToSouthFreeCardLib，可能为空（模拟异常时）
     */
    public static List<ToSouthFreeCardLib> simulateOneGame(int poolId, SouthernMoneyCfg moneyCfg) {
        List<ToSouthFreeCardLib> result = new ArrayList<>();
        ...
        for (int i = 0; i < SEAT_COUNT; i++) {
            ...
            Pair<Integer, List<Integer>> instantWin = ToSouthFreeHandUtils.getInstantWinCards(handCards);
            if (instantWin != null) {
                hasInstantWin = true;
                break;
            }
    ...
    private static void simulatePlayAndSettle(List<List<Integer>> seatCards,
            ...
            if (isLeader) {
                ...
                if (isFirstRound) {
                    ...
                    if (spade3 != null) {
                        ...
                        playCards = ToSouthFreeHandUtils.findBestPlayWithFirstCard(rankMap, spade3);
                    }
                }
                if (playCards == null) {
                    playCards = ToSouthFreeHandUtils.findBestPlay(handCards);
                }
                ...
                if (lastPlayCardIds != null) {
                    ...
                    playCards = ToSouthFreeHandUtils.findBestFollowPlay(handCards, lastCards);
                }
            }

            if (CollUtil.isNotEmpty(playCards)) {
                // 出牌成功
                ToSouthFreeCardType cardType = ToSouthFreeHandUtils.getCardType(playCards);
        ...
        // 计算最终结算倍数（与ToSouthFreeSettlementPhase.calSettlement一致）
        ...
        for (int loserSeat : loserSeats) {
            ...
            int countTwo = ToSouthFreeHandUtils.countTwo(loserHandCards);
            int countRedTwo = ToSouthFreeHandUtils.countRedTwo(loserHandCards);
    ...
    /**
     * 模拟炸弹结算（简化版，复用ToSouthFreeGameController的核心逻辑）
     */
    ...
    /**
     * 计算被炸牌型的赔付倍数（与ToSouthFreeGameController.getBombSettlementMultiplier一致）
     */
    private static long getBombMultiplier(SimRoundRecord record,
        ...
        cards.sort(ToSouthFreeHandUtils.CARD_COMPARATOR);

        boolean allRank2 = cards.stream().allMatch(c -> c.getRank() == RANK_2);
        if (allRank2 && (record.cardType == ToSouthFreeCardType.SINGLE
                || record.cardType == ToSouthFreeCardType.PAIR
                || record.cardType == ToSouthFreeCardType.TRIPLE)) {
            long multi = 0;
            for (Card c : cards) {
        ...
        if (record.cardType == ToSouthFreeCardType.BOMB_QUAD) {
            return moneyCfg.getFourkindboom();
        }
        if (record.cardType == ToSouthFreeCardType.CONSECUTIVE_PAIRS) {
    ...
    private static boolean isBomb(ToSouthFreeCardType type) {
     ...
     * 构建牌库条目
     */
    private static ToSouthFreeCardLib buildCardLib(long multiplier, int targetSeat,
        ...
        return new ToSouthFreeCardLib(multiplier, playerCards, robotCards);
    }
    ...
    private static class SimRoundRecord {
        int seatId;
        ToSouthFreeCardType cardType;
        List<Integer> cards; // pokerPoolId

        SimRoundRecord(int seatId, ToSouthFreeCardType cardType, List<Integer> cards) {
            this.seatId = seatId;
