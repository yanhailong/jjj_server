 ...
 * @author Auto
 */
public class ToSouthFreeRobotHandler extends BasePokerRobotProcessorHandler<ToSouthFreeGameDataVo> {

    private static final Logger log = LoggerFactory.getLogger(ToSouthFreeRobotHandler.class);
    ...
    public ToSouthFreeRobotHandler(GameRobotPlayer gameRobotPlayer, int type,
                               BasePokerGameController<ToSouthFreeGameDataVo> gameController, int readyPro) {
    ...
    @Override
    public void action() {
        BasePokerGameController<ToSouthFreeGameDataVo> gameController = getGameController();
        if (gameController instanceof ToSouthFreeGameController controller) {
            switch (getType()) {
                ...
                default -> log.warn("ToSouthFreeRobotHandler 未知行为类型: {}", getType());
            }
        }
    }
}
