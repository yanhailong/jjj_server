...
import com.jjg.game.poker.game.tosouthblood.message.notify.NotifyToSouthBloodSettlementInfo;
...
import com.jjg.game.poker.game.tosouthblood.room.data.ToSouthBloodGameLog;
...
public class ToSouthBloodSettlementPhase extends BaseSettlementPhase<ToSouthBloodGameDataVo> {
    ...
    private final ToSouthBloodSettlementContext context;
    private final List<PlayerSeatInfo> winners;

    public ToSouthBloodSettlementPhase(AbstractPhaseGameController<Room_ChessCfg, ToSouthBloodGameDataVo> gameController, ToSouthBloodSettlementContext context) {
...
//                                26000 - 8000
    ...
    private void calSettlement(ToSouthBloodGameDataVo gameDataVo, Map<Long, Long> settlementMap, long baseBet, SouthernMoneyCfg moneyCfg) {
        ...
        for (PlayerSeatInfo loser : losers) {
            ...
            log.debug("计算输家 {} 分数 - 剩余手牌: {}", loser.getPlayerId(), ToSouthBloodHandUtils.cardListToString(handCards));
     ...
     * 同时持久化到Redis（跨房间保留）
     */
    private void updatePlayerWinStreak(ToSouthBloodGameDataVo gameDataVo, Map<Long, Long> settlementMap2) {
        Map<Long, Integer> streakMap = gameDataVo.getPlayerWinStreakMap();
        Map<Long, Long> profitMap = gameDataVo.getPlayerTotalProfitMap();

        ToSouthBloodCardLibManager cardLibManager = null;
        try {
            cardLibManager = CommonUtil.getContext().getBean(ToSouthBloodCardLibManager.class);
        } catch (Exception e) {
            log.error("获取ToSouthBloodCardLibManager异常", e);
        }
     ...
     */
    private void updatePoolBalance(ToSouthBloodGameDataVo gameDataVo, Map<Long, Long> settlementMap2) {
        if (!(gameController instanceof ToSouthBloodGameController controller)) {
            return;
        }
        ToSouthBloodCardLibManager cardLibManager = null;
        try {
            ...
            log.error("获取ToSouthBloodCardLibManager异常，跳过水池更新", e);
            return;
        }
     ...
     */
    private void addLog(ToSouthBloodGameController controller, ToSouthBloodGameDataVo gameDataVo,
                        Map<Long, Long> settlementMap2, List<ToSouthBloodPlayerSettlementInfo> playerSettlementInfos) {
        // 1. 游戏级数据：完整流程事件列表（结构化对象，FastJSON 会序列化为 JSON 数组）
        gameDataTracker.addGameLogData("toSouthBloodInfo", gameDataVo.getGameLog().getEvents());

        // 2. 玩家级数据
        // 从结算信息中构建 playerId -> finalWinScore 映射
        Map<Long, Long> winScoreMap = new HashMap<>();
        for (ToSouthBloodPlayerSettlementInfo info : playerSettlementInfos) {
     ...
     * 好友房：计算房主收益并记录流水
     */
    private void addCreateRecord(ToSouthBloodGameController controller, long totalTax, Map<Long, Long> settlementMap2) {
