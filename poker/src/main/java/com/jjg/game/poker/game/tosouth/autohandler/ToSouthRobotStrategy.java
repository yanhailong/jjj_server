 ...
 * <p>
 * 复用 {@link ToSouthHandUtils} 的工具方法（integrateHandCards / findAllFollowPlays 等），
 * 不改动游戏规则。
 */
public class ToSouthRobotStrategy {

    private static final Logger log = LoggerFactory.getLogger(ToSouthRobotStrategy.class);
    ...
    private static class HandContext {
        ...
        final Map<ToSouthCardType, List<List<Card>>> integrated;
        ...
        HandContext(List<Card> handCards) {
            ...
            this.rankMap = ToSouthHandUtils.convertCardListToRankMap(handCards);
            this.integrated = ToSouthHandUtils.integrateHandCards(handCards);

            this.bombs = getOrEmpty(integrated, ToSouthCardType.BOMB_QUAD);
            this.straights = getOrEmpty(integrated, ToSouthCardType.STRAIGHT);
            this.triples = getOrEmpty(integrated, ToSouthCardType.TRIPLE);
            this.pairs = getOrEmpty(integrated, ToSouthCardType.PAIR);
            this.singles = getOrEmpty(integrated, ToSouthCardType.SINGLE);
            this.consecPairs = getOrEmpty(integrated, ToSouthCardType.CONSECUTIVE_PAIRS);
        ...
        private static List<List<Card>> getOrEmpty(Map<ToSouthCardType, List<List<Card>>> map, ToSouthCardType type) {
     ...
     */
    public static List<Card> chooseLeaderPlay(List<Card> handCards, ToSouthGameDataVo gameDataVo, PlayerSeatInfo mySeatInfo) {
    ...
    public static List<Card> chooseFollowPlay(List<Card> handCards, List<Card> lastCards,
                                              ToSouthGameDataVo gameDataVo, PlayerSeatInfo mySeatInfo) {
        ...
        if (lastType == ToSouthCardType.NONE) return null;
    ...
    private static List<Card> handleFirstRoundLeader(HandContext ctx, List<Card> handCards) {
        ...
        if (spade3 == null) {
            ...
            return ToSouthHandUtils.findBestPlay(handCards);
        }
        ...
        return ToSouthHandUtils.findBestPlayWithFirstCard(rankMap, spade3);
    }
    ...
    private static List<Card> tryPlayTwos(HandContext ctx) {
        ...
        twos.sort(ToSouthHandUtils.CARD_COMPARATOR);
    ...
    private static List<Card> followStraight(HandContext ctx, List<Card> lastCards) {
        List<List<Card>> allFollows = ToSouthHandUtils.findAllFollowPlays(ctx.handCards, lastCards);
        List<List<Card>> straightFollows = filterByType(allFollows, ToSouthCardType.STRAIGHT);
    ...
    private static List<Card> followTriple(HandContext ctx, List<Card> lastCards, int opponentHandSize) {
        List<List<Card>> allFollows = ToSouthHandUtils.findAllFollowPlays(ctx.handCards, lastCards);
        List<List<Card>> tripleFollows = filterByType(allFollows, ToSouthCardType.TRIPLE);
    ...
    private static List<Card> followPair(HandContext ctx, List<Card> lastCards, int opponentHandSize) {
        List<List<Card>> allFollows = ToSouthHandUtils.findAllFollowPlays(ctx.handCards, lastCards);
        List<List<Card>> pairFollows = filterByType(allFollows, ToSouthCardType.PAIR);
    ...
    private static List<Card> followSingle(HandContext ctx, List<Card> lastCards) {
        List<List<Card>> allFollows = ToSouthHandUtils.findAllFollowPlays(ctx.handCards, lastCards);
        List<List<Card>> singleFollows = filterByType(allFollows, ToSouthCardType.SINGLE);
    ...
    private static List<Card> followConsecutivePairs(HandContext ctx, List<Card> lastCards) {
        List<List<Card>> allFollows = ToSouthHandUtils.findAllFollowPlays(ctx.handCards, lastCards);
        List<List<Card>> cpFollows = filterByType(allFollows, ToSouthCardType.CONSECUTIVE_PAIRS);
    ...
    private static List<Card> followWithBomb(HandContext ctx, List<Card> lastCards) {
        ...
        List<List<Card>> bombFollows = allFollows.stream()
                .filter(f -> {
                    ToSouthCardType t = ToSouthHandUtils.getCardType(f);
                    return t == ToSouthCardType.BOMB_QUAD || t == ToSouthCardType.CONSECUTIVE_PAIRS;
                })
                .collect(Collectors.toList());
     ...
     */
    private static boolean isLastPlayTwoOrBomb(List<Card> lastCards, ToSouthCardType lastType) {
        // 炸弹类型
        if (lastType == ToSouthCardType.BOMB_QUAD || lastType == ToSouthCardType.CONSECUTIVE_PAIRS) {
            return true;
        }
        // 2的牌型（单2、对2、三个2）
    ...
    /** 获取指定座位玩家的手牌数量 */
    private static int getPlayerHandSize(ToSouthGameDataVo gameDataVo, int seatId) {
    ...
    /** 是否有对手手牌只剩1张（报单状态） */
    private static boolean hasAlertPlayer(ToSouthGameDataVo gameDataVo, PlayerSeatInfo mySeatInfo) {
        for (PlayerSeatInfo info : gameDataVo.getPlayerSeatInfoList()) {
    ...
    /** 全场所有人手牌 < 5 张 */
    private static boolean allPlayersUnder5Cards(ToSouthGameDataVo gameDataVo) {
        for (PlayerSeatInfo info : gameDataVo.getPlayerSeatInfoList()) {
     ...
     * 按牌型过滤跟牌候选列表
     */
    private static List<List<Card>> filterByType(List<List<Card>> plays, ToSouthCardType type) {
        return plays.stream()
                .filter(p -> ToSouthHandUtils.getCardType(p) == type)
                .collect(Collectors.toList());
    }
    ...
    private static List<Card> getSmallest(List<List<Card>> groups) {
        ...
        return groups.stream()
                .min((a, b) -> {
                    ...
                    return ToSouthHandUtils.CARD_COMPARATOR.compare(maxB, maxA);
                })
                .orElse(null);
    }
