package com.jjg.game.poker.game.tosouthblood.manager;
...
@Component
public class ToSouthBloodStartManager implements IRoomStartListener, GmListener {
    private static final Logger log = LoggerFactory.getLogger(ToSouthBloodStartManager.class);

    @Autowired
    private ToSouthBloodCardLibManager toSouthBloodCardLibManager;
    ...
    private CommonResult<String> handleAddWinStreak(PlayerController playerController, String[] gmOrders) {
        ...
        long playerId = playerController.playerId();
        int oldStreak = toSouthBloodCardLibManager.getPlayerWinStreak(playerId);
        int newStreak = oldStreak + delta;

        // 1. 更新Redis
        toSouthBloodCardLibManager.setPlayerWinStreak(playerId, newStreak);
        ...
        try {
            ...
            if (gc instanceof ToSouthBloodGameController tgc) {
    ...
    private CommonResult<String> handleAddTotalProfit(PlayerController playerController, String[] gmOrders) {
        ...
        if (!(gc instanceof ToSouthBloodGameController tgc)) {
            res.code = Code.FAIL;
            res.data = "玩家不在南方前进-血战房间中，无法修改水池";
            return res;
        }
        int roomCfgId = tgc.getRoom().getRoomCfgId();

        long oldBalance = toSouthBloodCardLibManager.getPoolBalance(roomCfgId);
        long newBalance = toSouthBloodCardLibManager.addPoolBalance(roomCfgId, delta);

        // 获取修改后匹配的调控模型ID
        PoolResultsCfg matchedCfg = toSouthBloodCardLibManager.selectPoolResultsCfg(roomCfgId);
        int modelId = matchedCfg != null ? matchedCfg.getModelId() : -1;
        long poolDiff = toSouthBloodCardLibManager.getPoolDiff(roomCfgId);
    ...
    @Override
    public void start() {
        log.info("正在启动南方前进-血战-血战游戏...");
        toSouthBloodCardLibManager.initPool();
    }

    @Override
    public void shutdown() {
        log.info("正在关闭南方前进-血战-血战游戏...");
        GM_DEAL_CARDS.clear();
