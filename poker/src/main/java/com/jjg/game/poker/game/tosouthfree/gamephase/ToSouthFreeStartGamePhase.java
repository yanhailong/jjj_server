...
import com.jjg.game.poker.game.tosouthfree.cardlib.ToSouthFreeCardLib;
...
import com.jjg.game.poker.game.tosouthfree.manager.ToSouthFreeStartManager;
...
/**
 * 南方前进-免费开始游戏阶段 (洗牌发牌动画)
 */
public class ToSouthFreeStartGamePhase extends BaseStartGamePhase<ToSouthFreeGameDataVo> {
    ...
    public void phaseDoAction() {
        ...
        if (gameController instanceof ToSouthFreeGameController controller) {
            ...
            log.debug("南方前进-免费开始游戏，房间底注为：{}", warehouseCfg.getBetShow());

            // 1. 洗牌发牌
            Map<Integer, PokerCard> cardListMap = ToSouthFreeDataHelper.getCardListMap(ToSouthFreeDataHelper.getPoolId(gameDataVo));
            if (BOMB_TEST_MODE) {
            ...
            if (samePlayers) {
                ...
                if (firstPlayer != null && !firstPlayer.isDelState()) {
                    ...
                    if (firstPlayer == null) {
                        log.warn("南方前进-免费牌组中没有黑桃3，请检查配置");
                        return;
                    }
                    gameDataVo.setIndex(firstPlayer.getSeatId());
                    gameDataVo.setRoundLeaderSeatId(firstPlayer.getSeatId());
    ...
    private void sendCards(Map<Integer, PokerCard> cardListMap, ToSouthFreeGameDataVo gameDataVo) {
        ...
        for (PlayerSeatInfo info : gameDataVo.getPlayerSeatInfoList()) {
            ...
            // 处理玩家自身的GM手牌
            List<int[]> gmCards = ToSouthFreeStartManager.consumeGmCards(info.getPlayerId());
            ...
            // 处理该玩家发起的机器人GM手牌
            Map<Integer, List<int[]>> robotCardsMap = ToSouthFreeStartManager.consumeGmRobotCards(info.getPlayerId());
        ...
        for (PlayerSeatInfo info : gameDataVo.getPlayerSeatInfoList()) {
            ...
            List<Integer> highlightIds = ToSouthFreeHandUtils.sortAndGetHighlightCards(handCards);
            ...
            if (log.isDebugEnabled()) {
                ...
                log.debug("发牌 - 玩家: {}, 座位: {}, 手牌: {}{}", info.getPlayerId(), info.getSeatId(),
                        ...
                        ToSouthFreeHandUtils.cardListToString(handCards));
            }
            // 记录发牌到一局日志
     ...
     */
    private boolean tryDealFromCardLib(Map<Integer, PokerCard> cardListMap, ToSouthFreeGameDataVo gameDataVo) {
        if (!(gameController instanceof ToSouthFreeGameController controller)) {
            return false;
        }

        // 获取 ToSouthFreeCardLibManager
        ...
        // ====== 从Redis抽取牌库 ======
        ToSouthFreeCardLib cardLib = cardLibManager.getCardLib(selectedSectionKey);
        if (cardLib == null) {
        ...
        for (PlayerSeatInfo info : activePlayers) {
            ...
            // 记录发牌到一局日志
            gameDataVo.getGameLog().recordDeal(info.getPlayerId(), info.getSeatId(),
                    "[Card Library] " + ToSouthFreeHandUtils.cardListToString(handCards));
     ...
     * 用于判断是否跳过牌库抽牌
     */
    private boolean hasAnyGmCards(ToSouthFreeGameDataVo gameDataVo) {
        for (PlayerSeatInfo info : gameDataVo.getPlayerSeatInfoList()) {
            if (info.isDelState()) continue;
            long playerId = info.getPlayerId();
            if (ToSouthFreeStartManager.hasGmCards(playerId) || ToSouthFreeStartManager.hasGmRobotCards(playerId)) {
                return true;
            }
        }
        return false;
     ...
     */
    private void sendBombTestCards(Map<Integer, PokerCard> cardListMap, ToSouthFreeGameDataVo gameDataVo) {
        // 按点数分组，2单独收集作为填充牌
    ...
    private PlayerSeatInfo findSeatWithSpecifyCard(ToSouthFreeGameDataVo gameDataVo, Map<Integer, PokerCard> cardMap, int rank, int suit) {
    ...
    private void checkInstantWin(ToSouthFreeGameController controller) {
        ToSouthFreeGameDataVo gameDataVo = controller.getGameDataVo();
        ...
        for (PlayerSeatInfo seatInfo : gameDataVo.getPlayerSeatInfoList()) {
            ...
            if (log.isDebugEnabled()) {
                ...
                log.debug("通杀检查 - 玩家: {}, 手牌: {}", seatInfo.getPlayerId(), ToSouthFreeHandUtils.cardListToString(sorted));
            }
            Pair<Integer, List<Integer>> instantWinCards = ToSouthFreeHandUtils.getInstantWinCards(handCards);
            if (instantWinCards != null) {
                ...
                gameDataVo.getGameLog().recordInstantWinSettlement(seatInfo.getPlayerId(),
                        instantWinCards.getFirst(), ToSouthFreeHandUtils.cardListToString(sorted));
            }
        }

        if (!winnerMap.isEmpty()) {
            instantWinContext = new ToSouthFreeSettlementContext();
            ...
            for (Map.Entry<PlayerSeatInfo, Pair<Integer, List<Integer>>> entry : winnerMap.entrySet()) {
                ...
                instantWinContext.addItem(new ToSouthFreeSettlementContext.SettlementItem(
                        entry.getKey(),
                        true,
                        winCards.getFirst(),
                        winCards.getSecond()
    ...
    public void nextPhase() {
        if (gameController instanceof ToSouthFreeGameController controller) {
            if (instantWinContext != null && !instantWinContext.getWinners().isEmpty()) {
                // 通杀，直接进入结算
                controller.addPokerPhaseTimer(new ToSouthFreeSettlementPhase(controller, instantWinContext));
            } else {
                // 进入打牌阶段
                controller.addPokerPhaseTimer(new ToSouthFreePlayCardPhase(controller));
            }
        }
    }
}
