 ...
 */
public record PlayerRechargeEventHandler(long playerId, AbstractGameController<?, ?> gameController,
    ...
    public void action() throws Exception {
        try {
            ...
            if (gamePlayer == null) {
                log.error("玩家充值时修改玩家vip等级失败 gamePlayer为空 playerId:{} order:{} ", playerId, JSON.toJSONString(order));
                return;
            }
            VipCheckManager.rechargeCheckVipLevel(gamePlayer, order.getPrice());
            //修改vip等级和经验
            ...
            log.error("玩家充值时修改玩家vip等级失败 playerId:{} order:{} ", playerId, JSON.toJSONString(order), e);
        }
    }
}
