...
//            ClusterClient client = clusterSystem.randClientByType(NodeType.HALL);
     ...
     * 后台完成玩家模拟经营新手引导。
     */
    @RequestMapping(BackendGMCmd.FINISH_SIM_GUIDE)
    public WebResult<String> finishSimGuide(@RequestBody FinishSimGuideDto dto) {
        log.info("收到后台完成新手引导请求 dto={}", dto);
        try {
            if (dto == null || dto.playerId() <= 0) {
                return fail("common.paramerror");
            }

            List<Integer> guideIds = null;
            if (dto.operationType() == BackendGMCmd.SimGuideOperation.FINISH_SPECIFIED) {
                if (StringUtils.isBlank(dto.guideIds())) {
                    return fail("common.paramerror");
                }
                LinkedHashSet<Integer> uniqueIds = new LinkedHashSet<>();
                for (String value : dto.guideIds().split(",", -1)) {
                ...
                guideIds = new ArrayList<>(uniqueIds);
            } else if (dto.operationType() != BackendGMCmd.SimGuideOperation.FINISH_ALL) {
                return fail("common.paramerror");
            }

            PlayerSessionInfo sessionInfo = playerSessionService.getInfo(dto.playerId());
            if (sessionInfo == null || StringUtils.isBlank(sessionInfo.getCurrentNode())) {
                log.warn("后台完成新手引导失败，玩家不在线或模拟经营数据未加载 playerId={}", dto.playerId());
                return fail("common.fail");
            }
            ClusterClient clusterClient = clusterSystem.getClusterByPath(sessionInfo.getCurrentNode());
            if (clusterClient == null || !NodeType.HALL.toString().equals(clusterClient.getType())) {
                log.warn("后台完成新手引导失败，玩家当前不在大厅节点 playerId={},currentNode={}",
                        dto.playerId(), sessionInfo.getCurrentNode());
                return fail("common.fail");
            }
            ...
            int code = gmToHallBridge.finishSimGuide(dto.playerId(), dto.operationType(), guideIds);
            ...
            log.warn("后台完成新手引导参数格式错误 dto={}", dto, e);
            return fail("common.paramerror");
        } catch (Exception e) {
            log.error("后台完成新手引导异常 dto={}", dto, e);
            return fail("common.exception");
        }
    }
