...
public class MatchService {
    ...
    private static final String TRY_JOIN_ROOM_SCRIPT = """
            local function createRoom()
                ...
                if result == 1 then
                    ...
                    local seconds = tonumber(time[1]) % 4294967296
    ...
    public long getWaitingRoomId(int gameType, int roomConfigId, int maxPlayer, String nodePath) {
        String matchRedisKey = matchDataDao.getMatchRedisKey(gameType, roomConfigId, nodePath);
     ...
     * 添加到等待房间ID
     */
    public void addWaitingRoomId(int gameType, int roomConfigId, long roomId, long roomCreateTime, String nodePath) {
        matchDataDao.addWaitJoinRoomId(gameType, roomConfigId, roomId, roomCreateTime, nodePath);
    }
