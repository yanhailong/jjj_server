...
public class HallService implements ConfigExcelChangeListener {
    ...
    public CommonResult<Integer> bindPhone(long playerId, String data) {
        ...
        if (data.equals(account.getPhoneNumber())) {
            result.code = Code.REPEAT_OP;
            log.debug("玩家当前绑定手机号与新手机号一致，绑定手机号失败 playerId = {},oldPhone = {},newPhone = {}", playerId,
                account.getPhoneNumber(), data);
            return result;
        }
    ...
    public CommonResult<Integer> bindEmail(long playerId, String data) {
        ...
        if (data.equals(account.getEmail())) {
            result.code = Code.REPEAT_OP;
            log.debug("玩家当前绑定邮箱与新邮箱一致，获取绑定邮箱验证码失败 playerId = {},oldPhone = {},newPhone = {}", playerId,
                account.getPhoneNumber(), data);
            return result;
        }
...
//        }
    ...
    public CommonResult<PlayerPack> useItem(long playerId, int girdId, int itemId) {
        ...
        try {
            ...
            for (Map.Entry<Integer, Long> en : itemCfg.getGetItem().entrySet()) {
                ...
                useResult = playerPackService.useItem(playerId, girdId, itemId, addItemId, en.getValue(),
                    "packUseItem");
            }
    ...
    private void initGlobalConfig() {
        GlobalConfigCfg globalConfigCfg =
            GameDataManager.getGlobalConfigCfg(GameConstant.GlobalConfig.DEFAULT_AVATAR_CFG_ID);
