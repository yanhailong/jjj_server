...
//        clusterSystem.sessionMap().entrySet().forEach(en -> en.getValue().send(notify));
    ...
    public CommonResult<Integer> bindPhone(long playerId, String data) {
        ...
        VerCode vc = new VerCode(realPhone);
        vc.setPlayerId(playerId);
        vc.setVerCodeType(VerCodeType.SMS_BIND_PHONE);
        CommonResult<VerCode> verCodeCommonResult = verCodeDao.querySmsVerCodeInfo(vc);
        ...
        CommonResult<VerCode> sendCodeResult = smsService.sendCode(vc);
        ...
        result.data = sendCodeResult.data.getCode();
        return result;
    }
    ...
    public CommonResult<Integer> bindEmail(long playerId, String data) {
        ...
        result.code = verCodeDao.addMailVerCode(vc);
        result.data = verCode;
        return result;
    }
     ...
     * @return
     */
    public CommonResult<String> comfirmVerCode(Player player, int verCodeType, int verCode) {
        ...
        if (smsType == VerCodeType.SMS_BIND_PHONE) {
            ...
            if (!verResult.success()) {
                ...
                log.warn("绑定手机时验证码校验失败 playerId = {}", player.getId());
                return result;
            }
        ...
        if (smsType == VerCodeType.MAIL_BIND_MAIL) {
            ...
            if (!verResult.success()) {
                result.code = verResult.code;
                log.warn("绑定邮箱时验证码校验失败 playerId = {}", player.getId());
                return result;
            }
    ...
    private void updatePoolEvent() {
        ...
        this.sortGameList.forEach(cfg -> {
            if (CommonUtil.getMajorTypeByGameType(cfg.sid) == CoreConst.GameMajorType.SLOTS) {
                ...
                tmpSvipPoolMap.put(cfg.sid, sipWarePoolInfoList);
            }
        });
